...
### A-2. 「メールアドレスにつき1アカウント」にする（重要・2アカウント化を防ぐ）

同じメールアドレスなのに「メールで登録したアカウント」と「Googleで入ったアカウント」が**別々にできてしまう**のを防ぐ設定です。

1. Authentication →「**Settings（設定）**」タブ → 「**User account linking（アカウントのリンク）**」。
2. 「**Link accounts that use the same email（同じメールを使うアカウントをリンク）**」＝**「メールアドレスにつき1つのアカウント」**を選択して保存。
   - もう一方の「Create multiple accounts...（メールアドレスごとに複数アカウントを許可）」だと**2アカウントに分かれます**。必ず前者に。

> この設定にすると、すでにメール/パスワードで登録済みのメールでGoogleログインしようとしたとき、コード側が
> 「そのメールでログイン→自動でGoogleを連携」する案内に切り替えます（下の D 参照）。**アカウントは1つに保たれます。**
>
> ⚠️ すでに**同じメールで2つに分かれてしまったアカウント**は、この設定を変えても**自動では統合されません**（Firebaseの仕様）。
> 片方（残したい方）でログインし直して使ってください。どうしても統合したい場合はコンソールの Authentication → Users から不要な方を削除します。
...
> **ログイン時のデータ同期の挙動**：
> - そのアカウントにクラウド保存が**無ければ**、いまの端末（ゲスト）の記録をアップロードして引き継ぎます。名前が空ならGoogleの表示名を初期値にします。
> - すでにクラウド保存があり、端末側に記録が**無い/同じ**なら、クラウドを復元します。
> - **両方に記録があって食い違うとき**（例：ゲストで遊んで貯めたトロフィーと、そのアカウントの記録が違う）は、
>   「☁ クラウドの記録を呼び出す／📱 この端末の記録で上書きする」の**確認ダイアログ**を出し、選んだ方に統一します。
>   → これにより「ログインしたらトロフィー/勝利数が勝手にリセットされる」事故を防ぎます。
>
> **ゲスト（未ログイン）**の記録も端末（localStorage）に残るので、リロードしても消えません。ログイン/連携したときだけクラウドと同期します。
