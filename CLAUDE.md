...
- **大福サムライの武器合成（刀・鞘の分離素材）**：未強化・スキン無しのときは `spriteFor` が**本体のみ**`daifuku_body_blue/red` を返し、`render` の `drawDaifukuWeapon(u,S,cx,cy)` が**鞘 `daifuku_sheath`（腰に固定）＋刀 `daifuku_katana`（状態別に回転/移動）**を上乗せ描画。刀のポーズは `u.cstate`：`charge`=納刀→溜め終盤で抜刀構え（鞘から出て上がる）／`dash`=抜刀＝突進方向(`ddx,ddy`)へ突き出す／`walk`+`atkAnim`=通常攻撃の振り下ろし弧／その他=納刀。回転軸は刀スプライト内 `KAT_PIVX,KAT_PIVY`(0.24,0.5)＝柄。強化(ちょんまげ大福)・スキンは従来の一枚絵にフォールバック。
