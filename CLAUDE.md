...
- **ゼリースライムのぽよんジャンプ移動**（`slime`）：`stepWorld` 末尾で `u.mv`（移動フラグ＝前フレームからの移動量 > `SLIME_MOVE_EPS`。融合中は常に false）を立て、**移動中だけ**3フレームで跳ねる＝「潰れ `slime_squash_*`（接地・幅広）→ 伸び `slime_up_*`（上昇/下降・縦長）→ 頂点は**通常 `slime_blue/red`**（伸びも潰れもしない瞬間）→ 伸び → 潰れ」。位相は純関数的な `slimeHopPhase(u)`＝`(world.t*SLIME_HOP_FPS + u.x*0.03)%1`（`u.x` で個体ごとに位相ずらし）で、`spriteFor` は 位相<0.15/>0.85＝squash・0.40〜0.60＝通常（頂点）・それ以外＝up を返す。**縦の跳ね**は `render` が `slimeHopLift(u)`（`sin(位相*π)`＝接地0〜頂点1のアーチ）× `S*SLIME_HOP_H` を**本体スプライトだけ**持ち上げる（足元リング/影は地面に残る＝跳ねてる感）。止まっているとき＝通常 `slime_blue/red`、融合＝`slime_*_big`（静止）。差分は元絵64pxを地面ベースライン(y=122)でボトムアンカー＆水平センタリングして`SPRITE_DATA`に128pxで注入（squash=2×／up=幅2.9×高さ3.0×で太め縦長／青は直・赤は青→赤recolor）。`mv` は `SNAP_UNIT_FIELDS` に含めPVPの子へ伝える（子は `world.t`/`u.x` を持つので絵と跳ねが一致）。調整＝`SLIME_HOP_FPS`(跳ね頻度・現1.3)/`SLIME_HOP_H`(高さ)/`SLIME_MOVE_EPS`(移動判定)。テスト：test.js 119。
