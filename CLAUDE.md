...
| ghost | わたあめゴースト | 3.5 | 3 | 16 | 40 | 95 | 開幕少し待って敵後方へワープ（warpDelay1.5・無敵中は狙われない＆動かない） |
| cannon | キャンディキャノン | 3 | 1 | 0 | 140 | 0 | 不動の全域誘導AoE迫撃（手前の敵を優先・`mortar`=90/`cd`=4.8・爆発範囲 `splash`=35）。強化「ぱちぱちキャンディ」=着弾で小爆発を撒く |
...
**近日登場（カミングスーン）**：`COMING_SOON`（`[{sil:'sil_canule'},{sil:'sil_shortcake'}]`）に載せると、**編成のキャラ一覧末尾（自分の編成のみ）と指南のキャラ一覧末尾に、黒シルエット＋「🔜 カミングスーン」だけ**を出す（名前/性能は伏せる）。シルエットは `SPRITE_DATA` の `sil_<key>`（元絵を暗いスレート色1色に統一・背景透過・128px）。実装が済んだら `test:true` を外して各配列/レッスンに載せ、`COMING_SOON` から外す。
...
- **戦闘エンジン**：`createWorld` / `stepWorld(world,dt)`（phase=muster|battle|outro。毎フレームshuffleで左右バイアス除去）/ `nearestEnemy`（**無敵中`invuln>0`の敵は標的にしない**＝ワープ直後のゴーストにサムライ等が吊られない）/ `applyHit`（無敵中は無効。ノックバックは重量耐性＋上限＋無効時間。吸引中/起爆中は無反動）/ `killUnit`（爆発・スライム分裂・炭酸沼発生を一本化。爆発AoEも無敵中は当たらない）。**`noCollide`**＝当たり判定なし（分離/重なり解消の全ループで素通し＝壁にならない。イチゴタレット＝`untargetable`由来／マシュマロエンジェル＝`noCollide:true`。`makeFighters` が `untargetable||noCollide` から算出）。**赤リカラーの発色統一**：新規アニメの赤フレームは基準赤（`slime_red`/`macaron_red`＝ソフトな赤 g=b≈r×0.34〜0.58）に合わせ、赤ピクセルの g,b を r から `(160,55)(229,111)(240,138)` ランプで再計算（従来の`b→r`式は発色が強すぎてアニメ切替で一瞬赤が浮くため）。テスト：test.js 125（noCollide）。
- **専用挙動**：`chargerStep`(大福) / `artilleryStep`(キャノン) / `vacuumStep`(ドーナッツ) / `spawnerStep`(ベーカリー：`spawnerId` で個体ごとに独立した召喚枠) / `evolvePancake`・`evolvedStep`(パンケーキ進化＋ジャンプ衝撃波) / ワープ(ゴースト) / `shellStep`(マカロン：殻スピン→スタン→通常／true返却でその場処理) / `burrowStep`(カヌレモーラー：HP130/atk20/cd0.7・地上速度50(`speed`)／土中速度90(`digSpeed`)・隊列tier1.25＝パンケーキの後ろ・1枚で2体。開幕`MOLE_DIVE_DELAY`秒 地上で溜め→土に潜る＝`invuln`+`noCollide`で無敵&非ターゲット&すり抜けのまま`digSpeed`で敵へ掘り進む→`MOLE_EMERGE_R`まで接近で`moleEmerge`浮上＝false返却で通常近接戦。**浮上後に狙う敵が`MOLE_REDIG_R`(60)より遠ざかったら**（初回の`MOLE_DIVE_DELAY`溜めなしで）**即座に再潜行(dig)して追いかける**（ヒステリシス）。潜行中は`render`が本体を隠して土の盛り上がりだけ描画。固有強化「ジャイアントクロー」(`buff_canule`/`applyCanuleBuff`/`u.canuleBuff`)＝HP200(`CANULE_BUFF_HP`)/攻撃30(`CANULE_BUFF_ATK`)＋**土から出る瞬間に近くの敵1体へ`CANULE_EMERGE_DMG`(40)の不意打ち**（`moleEmerge`が`applyHit`）。立ち絵は`canule_blue/red`／強化`canule_buff_*`・カード(`iconHTML`)も同立ち絵。**カヌレ同士は地中(潜行=invuln)の相手も察知できる**（`nearestEnemy`が`u.burrow&&o.burrow`のときだけ無敵スキップを免除＝地中の敵カヌレを追える。通常ユニットは従来どおり地中を狙えない）。`burrow`/`moleState`/`canuleBuff`は`SNAP_UNIT_FIELDS`済み。test.js 127・127b・127c・127d) / 氷弾のヒット時スローは `u.chillT`/`chillAmt`、炭酸沼は `world.puddles` を `stepWorld` で毎フレーム処理（DoT＋`slowMul`減速）。
...
- **ゼリースライムのぽよんジャンプ移動**（`slime`）：`stepWorld` 末尾で `u.mv`（移動フラグ＝前フレームからの移動量 > `SLIME_MOVE_EPS`。融合中は常に false）を立て、**移動中だけ**3フレームで跳ねる＝「潰れ `slime_squash_*`（接地・幅広）→ 伸び `slime_up_*`（上昇/下降・縦長）→ 頂点は**通常 `slime_blue/red`**（伸びも潰れもしない瞬間）→ 伸び → 潰れ」。位相は純関数的な `slimeHopPhase(u)`＝`(world.t*SLIME_HOP_FPS + u.x*0.03)%1`（`u.x` で個体ごとに位相ずらし）で、`spriteFor` は 位相<0.15/>0.85＝squash・0.40〜0.60＝通常（頂点）・それ以外＝up を返す。**縦の跳ね**は `render` が `slimeHopLift(u)`（`sin(位相*π)`＝接地0〜頂点1のアーチ）× `S*SLIME_HOP_H` を**本体スプライトだけ**持ち上げる（足元リング/影は地面に残る＝跳ねてる感）。**融合スライムも同じ3フレームで跳ねる**＝巨大版 `slime_bigsquash_*`（潰れ）/`slime_bigup_*`（伸び）/頂点は `slime_*_big`（巨大立ち絵）。止まっているとき＝通常 `slime_blue/red`（融合は `slime_*_big`）。差分は元絵64pxを地面ベースライン(y=122)でボトムアンカー＆水平センタリングして`SPRITE_DATA`に128pxで注入（通常squash=2×/up=幅2.9×高さ3.0×・巨大squash=2×/up=幅2.7×高さ2.8×で太め縦長／青は直・赤は青→赤recolor）。`mv` は `SNAP_UNIT_FIELDS` に含めPVPの子へ伝える（子は `world.t`/`u.x` を持つので絵と跳ねが一致）。調整＝`SLIME_HOP_FPS`(跳ね頻度・現1.3)/`SLIME_HOP_H`(高さ)/`SLIME_MOVE_EPS`(移動判定)。テスト：test.js 119。
- **ベリーケーキスネイル（内部キー `shortcake`・旧表示名「ショートケーキ」）**：`shortcake_blue`(ブルーベリー)/`shortcake_red`(いちご)＝イチゴを背負ったスネイル。**表示名は「ベリーケーキスネイル」**（コード内キー/スプライト名/コメントは `shortcake` のまま）。**タレット設置中**（`u.myTurret` 生存）はイチゴを下ろした `shortcake_deploy_*` に切替（背中のベリー＝タレットになって飛び出した表現）。**戦闘中(`world.phase==='battle'`)は歩行2フレーム**＝背負い `shortcake_blue`/`shortcake2_blue`・設置中 `shortcake_deploy_blue`/`shortcake_deploy2_blue` を `SHORTCAKE_WALK_FPS`(4)で切替（**下の足の並々が波打つ**・`u.x`で個体位相ずらし）／待機・カードは frame1。地面に接地する普通のユニット描画（旧「本体だけ浮遊＋影別描画」＝`shortcake_shadow`は撤廃）。青=ブルーベリー/赤=いちご（アップ元絵はいちご＝赤基準で、青は**イチゴのピクセルだけ**＝`r-g>65`の濃い赤のみを**参考カラーの明るいコーンフラワー青(rgb87,126,235)へ輝度ランプで置換**し**クリーム色の体はそのまま**＝苺→ブルーベリー。単純なch入替でなく輝度→青ランプなので陰影が残る）。全4スネイル絵は同一bboxで64px→128pxにアンカー（背負い/下ろしで本体位置が揃う）。タレット `strawturret_blue`(ブルーベリー)/`strawturret_red`(いちご)。タレットは `untargetable`＝**狙われない＆当たり判定なし**（分離/重なり解消の全ループで `untargetable` を素通し＝敵の前に置いても「無敵の盾」にならない・test.js 125）。**固有強化「練乳ベリー」(`buff_shortcake`/`u.shortcakeBuff`)**＝設置するイチゴタレットの攻撃力(`SHORTCAKE_TURRET_ATK`9→11)・連射(`SHORTCAKE_TURRET_CD`0.15→0.11)を強化（`deployTurret` が owner の `shortcakeBuff` を見てタレットの atk/cd を差し替え＋`t.shortcakeBuff` を立てる）。**見た目も練乳がけに切替**＝タレットは `strawturret_buff_*`、本体は未設置時のみ `shortcake_buff_*`（背中の練乳ベリー／設置中は通常 deploy＝ベリーを下ろしている）。青は**イチゴのピクセルだけ**参考コーンフラワー青へ輝度ランプ（練乳=白/脚=緑は不変）。`shortcakeBuff` は `SNAP_UNIT_FIELDS` 済み。プリン等と同じ配線（`applyShortcakeBuff`/`eligibleSpecials`/`applyFlagBuffs`/`applyPvpSpecial`）。test.js 126。**アニメ動作確認**＝設定「🧪 アニメテスト」→ショートケーキを選ぶと `animTestLive` で味方青/敵赤を上下に置き `stepWorld` を回す＝前進→タレット設置→歩行/連射を実演。テスト：test.js 123。
...
- **クッキーアーミーの槍合成（槍・本体の分離素材）**：未強化(非パーティー)のとき `spriteFor` が**槍なし本体**`cookie_body_blue/red` を返し、`render` の `drawCookieSpear(u,S,cx,cy)` が槍 `cookie_spear_blue/red` を右手で握って状態別に回す（`cookieUsesParts`/`SP_PIVX,SP_PIVY`=握り0.5,0.72）。状態：**待機(muster)=槍を上に向ける**（今の一枚絵と同じ見た目）／**移動(battle中)=槍を左に倒す**(ang≈-1.15)／**攻撃(atkAnim>0)=左へ突き出す**（左へ平行移動＋ang≈-1.65）。赤(敵)版は青→赤 recolor で用意。**本体の歩行アニメ（足踏み2フレーム）**：`spriteFor` が戦闘中(`world.phase==='battle'`)は `cookie_body_*`(frame1＝待機/デフォルト)と `cookie_body2_*`(frame2)を毎秒6コマで切替（足だけ動く。`u.x`で個体ごとに位相をずらして揃わないように）。頭・腕・槍の握り位置は両フレーム共通bboxで揃えてあるので槍合成はズレない。強化「クッキーパーティー」(`u.party`)も**同じ合成＋歩行アニメ**＝サングラスの槍なし本体 `cookie_partybody_*`(frame1)/`cookie_partybody2_*`(frame2)を返し槍を合成（本体素材が無ければ従来の一枚絵`cookie_party_*`にフォールバック）。カード絵(iconHTML)＝通常は `cookie_*`／強化カードの絵(evoSprite)は一枚絵 `cookie_party_blue`（どちらも槍つき一枚絵で据え置き）。
...
- シェルマカロンは通常 `macaron_blue/red`／殻スピン・スタン中 `macaron_spin_blue/red` を `u.shellPhase` で切替（スピン中は描画を回転＋スタン中は★演出）。強化(マカロンアーマー/`u.macaronBuff`)は `macaron_buff_*`／`macaron_spin_buff_*`。**殻を脱いだ後(`shellPhase==='normal'`)の移動中は上の殻が「ぱかぱか」開閉**＝`spriteFor` が `MAC_PAKA_FPS`(5)で 閉`macaron_blue` ↔ 開`macaron_open_blue`(上の殻を持ち上げて口が開いた差分・64px元絵を`macaron_blue`の足元(y=114)・全幅に合わせて128pxで注入・赤はリカラー) を切替（`u.x`で個体位相ずらし・止まると閉じ）。移動判定 `u.mv` は `stepWorld` 末尾でスライムと共通に算出（`SLIME_MOVE_EPS`）、`SNAP_UNIT_FIELDS` 済みでPVPの子も一致。スピン/スタン中はぱかぱかせず閉じ殻。**強化(マカロンアーマー/`macaronBuff`)版も同じぱかぱか**＝閉`macaron_buff_*`↔開`macaron_open_buff_*`（`macaron_buff`の足元y=110に合わせて注入）。テスト：test.js 121。
...
- **シュークリームアーチャーの状態アニメ**（`shoe`）：`stepWorld` 末尾で `u.shoeSt`（0=歩行/1=構え/2=待機）を決める＝**移動中は歩行（`shoe_walk1/2_*` を `SHOE_WALK_FPS` で足バタバタ）／止まって発射直前（`u.cool<=SHOE_AIM_WINDOW`）は構え（矢あり `shoe_aim_*`）／それ以外は待機（矢なし `shoe_idle_*`）**。発射（`u.cool=cd`）した瞬間に待機へ戻る＝「構え→放つ→矢なし」。`spriteFor()` が `shoeSt`＋`world.t` でフレーム解決。**強化=特盛りも同じ4ポーズでアニメ**（`shoe_buff_aim/idle/walk1/walk2_*`／未用意なら旧一枚絵 `shoe_buff_*` にフォールバック）。カード立ち絵(`iconHTML`)＝構え `shoe_aim_*`／強化カード(`evoSprite`)＝`shoe_buff_aim_blue`。歩行速度は `SHOE_WALK_FPS`(4)。矢は `arrow_blue/red`（差し替え済み・render で進行方向へ回転）。`shoeSt` は `SNAP_UNIT_FIELDS` に含めPVPの子へ伝える。青→赤は弓のリカラー。テスト：test.js 118（faceと同ブロック）。
- **ポップコーンTNTの歩行アニメ**（`bomb`）：`spriteFor()` が**戦闘中(`world.phase==='battle'`)は歩行2フレーム**（`bomb_blue`/`bomb2_blue`＝frame1/2）を `BOMB_WALK_FPS`(7)で切替（足だけ動く。`u.x`で個体位相ずらし）／待機・カードは frame1。**強化「おかわり」(`u.spawnMini`)も同じ2枚歩行**＝`bomb_buff_blue`/`bomb_buff2_blue`（横にミニポップコーンが飛び出した差分）。frame2素材(`*2_*`)が無ければ frame1 に自動フォールバック。カード立ち絵＝`bomb_blue`／強化カード(`evoSprite`)＝`bomb_buff_blue`。青→赤は箱ストライプのリカラー（ポップコーン=クリーム/足=オレンジは不変）。`u.spawnMini` は既に `SNAP_UNIT_FIELDS` に含むのでPVPの子でも歩行＆強化差分が一致（`world.phase`/`world.t`/`u.x`も子が持つ）。8枚（通常/強化×青赤×frame1/2）を共通bbox・地面ベースライン(y=127)でアンカーして注入＝足だけ動き本体はブレない。テスト：test.js 120。
- **ランニングソーダの歩行アニメ**（`soda`）：ポップコーンと同じ仕組み＝`spriteFor()` が**戦闘中は歩行2フレーム**（`soda_blue`/`soda2_blue`）を `SODA_WALK_FPS`(8・俊足なので速め)で切替（`u.x`で個体位相ずらし）／待機・カードは frame1。**強化「炭酸沼強化」(`u.fizz`)も同じ2枚歩行**＝`soda_buff_blue`/`soda_buff2_blue`（太った缶）。frame2が無ければ frame1 にフォールバック。青→赤は缶ボディのリカラー（ラベル=白/足=オレンジは不変）。`u.fizz` は `SNAP_UNIT_FIELDS` 済みでPVPの子も一致。8枚を共通bbox・地面ベースライン(y=127)でアンカー注入。テスト：test.js 122。
...
- **【開発】テスト対戦（`?dev` メニュー「⚔️ テスト対戦」）**：`startTestBattle`→`#testBattleModal` で自分/相手CPUのデッキを自由選択（`test:true`のβキャラも可）→`beginTestBattle`→CPU戦で実戦確認（X2は無効＝`_pendingNoX2`／相手は round-robin で確実に出す `makeGuideFoeController`）。**「🔧 自分の強化を最初から全部適用」チェック**（`#tbForceBuff`）＝ONで `beginTestBattle` が `_tbForceBuffs` を立て、`startGame` が `resetState` 直後に `TB_BUFF_FLAGS`（全 `state.you*Buff`）を true にして1回消費＝ダブルスプーン等の固有強化をドラフトで引かずに即確認できる（`applyFlagBuffs` が毎ラウンド適用）。本来デッキは `_guideDeckBackup` に退避（保存しない）。
...
### PVP 子側のラグ差緩和（スナップショット補間＋外挿）— 実装済み

- **背景**：ホスト権威型なので**親＝60fpsで権威計算を即時／子＝スナップショット(戦闘中10Hz=`snapEvery`100ms)を遅延受信**という非対称がある。オートバトラーなので**勝敗結果は親が両軍計算＝完全同一**（有利不利には出ない）が、子の「見た目のなめらかさ」が悪いと体感が落ちる。
- **補間**：子は受信スナップショットを時刻つきで `pvpSnapBuf`（最大5枚）に貯め、**`PVP_SNAP_DELAY`(130ms)過去を描画**＝その時刻を挟む2スナップショット間を線形補間（`nid`＝ネットワークIDで同一ユニットを追跡し x/y/bob を lerp）。純粋関数 `pvpInterpFrame(buf,now,delay,maxExtrap)`（test.js 128）。
- **枯渇時の外挿（デッドレコニング）**：受信が追いつかず未来側スナップが無い瞬間は、**最新2点の速度で `PVP_MAX_EXTRAP`(120ms)だけ先読み外挿**してフリーズ→カクつきを防ぐ（上限つきで暴走しない）。過去すぎる時刻は最古で止める（外挿しない）。
- **ワープ除外**：1フレームで `PVP_JUMP_EPS`(90px)超の移動（ワープ/ラウンド切替）は補間・外挿とも素通し（瞬間移動をそのまま見せる）。
- **子のfps可変**：`pvpGuestLoop` は低負荷モード（`LOW_FX`）ON=約30fps／OFF=約60fps（`minFrame`＝28/16ms）。補間で中間フレームが埋まるので、非力端末は30fpsで軽く・通常端末は60fpsでなめらか。
- 調整＝`PVP_SNAP_DELAY`(遅延と滑らかさのトレード)／`PVP_MAX_EXTRAP`(先読み上限)／`PVP_JUMP_EPS`(ワープ判定)／送信側 `snapEvery`。テスト：test.js 128。
...
- **見た目3種（絵素材不要・CSS/テキスト）**：`FRAMES`（アイコン装飾枠＝box-shadow）／**二つ名＝`TITLE_PRE`＋`TITLE_SUF` の2パーツを組み合わせて作る**（`titleText()`＝装備中の1つ目＋2つ目・`titleTextOf(a,b)`＝任意組み合わせ）／`NAME_COLORS`（名前の色・グラデ）。各行に `unit`（テーマ表示用）。**【完全自由化】前半/後半の区別なく、どのパーツも1つ目・2つ目どちらのスロットにも装備できる**（`TITLE_PARTS=TITLE_PRE.concat(TITLE_SUF)`／`TITLE_PART_BY_ID` で解決。`equippedTitlePre/Suf`・`titleText`・`titleTextOf` は結合マップを参照）＝「大福ちょんまげ」「クッキーパーティー」なども作れる。装備は `myProfile.frame/titlePre/titleSuf/nameColor`＋`equipFrame/equipTitlePre/equipTitleSuf/equipNameColor`（トグル・所持のみ／`titlePre`=1つ目スロット・`titleSuf`=2つ目スロット・中身はどのパーツでも可）。表示は**自分のプロフィールチップ**（`renderProfileChip`）＋**プロフィール編集の見た目UI**（`renderProfileCosmetics`＝#authModal内・**開閉メニュー(アコーディオン)**＝カテゴリ(フレーム/1つ目/2つ目/カラー)ごとに開閉・両スロットとも `TITLE_PARTS` 全体から選ぶ・`_cosmOpen`/`toggleCosmeticSection`・二つ名プレビュー）＋VSカットイン。二つ名パーツはガチャ排出（`packPool` が `titlePre`/`titleSuf` を出す＝パンケーキ後半 `ts_pancake` 追加済み・`renderPackPool`(排出内容)は前半/後半の2セクション表示）。
...
- （作業中）PVP子側のラグ差緩和：枯渇時の外挿(デッドレコニング)＋fps可変(30/60)＋補間コアを純粋関数化(`pvpInterpFrame`・test.js 128)
...
現状の `sw.js` CACHE は **v68**。`node test.js` は **995 passed**。
...
- ~~**経済Phase 3（コインショップ）**：🍬で「先に解禁」するUI~~ → **✅ 実装済み**：編成のロックキャラをタップ→`openUnlockModal` の2択（🍬`unlockByCoins`→`buyUnit`＝一律500で即解禁／🎓`unlockByGuide`＝レッスンへ）。未到達レッスンのキャラも `buyUnit` で先取りできる。**追加でやるなら**：ショップ(`#shopModal`)側にも一覧型の「先に解禁」導線を置く／`unitUnlockCost` をキャラ・進捗別に調整、程度（必須ではない）。
