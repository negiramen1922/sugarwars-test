...
- **低負荷モード（弱い端末＝スマホ親のカクつき対策）**：`loop()` がフレーム時間のEMA（`_frameEMA`）を測り、重い端末では自動で `LOW_FX=true`（>30ms=約33fps未満でON／<20ms=約50fpsで戻す＝ヒステリシス）。ONのとき **見た目だけの粒子(`world.parts`) を削減**＝`burst()` の生成数を半減＋総数上限を `PART_CAP_HI`(200)→`PART_CAP_LO`(70) に下げ、`loop()` が毎フレーム古い粒子を間引く（`partCap()`）。炭酸沼はグラデ生成をやめフラット塗り。**CPU計算・当たり判定・スナップショットは不変＝ゲーム性/対戦の公平性に影響なし**（見た目の派手さだけ端末に合わせて自動調整）。強い端末は `LOW_FX=false` のまま従来通り。テスト：test.js 82。
