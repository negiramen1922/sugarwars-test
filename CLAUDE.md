...
- **チョコレートナイトの剣・盾合成（分離素材）**：未強化(非ビター装甲)のとき `spriteFor` が**本体のみ**`choco_body_blue/red` を返し、`render` の `drawChocoGear(u,S,cx,cy)` が**剣(左)`choco_sword_*`＋盾(右)`choco_shield_*`**を合成。**攻撃(atkAnim)＝剣を左に倒して振り下ろす**（`ang=-0.5-2.0*a`・握り`CSW_PIVX/Y`=0.5,0.82）／**歩行(battle)＝盾が上下に揺れる**（`Math.sin(u.bob)*S*0.07`）。赤は青→赤 recolor。強化(ビター装甲/`u.chocoBuff`)は従来の一枚絵`choco_buff_*`。カード絵は従来 `choco_*`。
