...
- **設定**：`AD_CONFIG={network,id}`（既定は `PASTE_` プレースホルダ＝未設定）。`adsConfigured()` が真のときだけショップにボタンが出る。ユーザーが広告ネットワーク（例 GameMonetize / GameDistribution / AdSense for games(H5)）でアカウント作成→キーを貼る。**Web用H5広告なので Androidアプリ(TWA)・iOSブラウザ両方に同じコードで効く**（AdMobネイティブSDKはTWA/ブラウザ不可）。
