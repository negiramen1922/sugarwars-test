...
- **アニメテストの動作確認（`ANIM_TEST_EXTRA`＝`strawturret`/`daifuku`）**：`animTestUnits()` に追加。選ぶと `animTestLive=true` で `animTestLoop` が `stepWorld` を回す（他キャラは従来どおり `world.t` だけ進めて静止アニメ確認）。タレット＝味方2基＋各射程内に不死・不動の敵で連射を確認。**サムライ＝味方1体＋上に不死の敵**を置き、`納刀→抜刀構え→抜刀(突撃)→通常攻撃`を実演。近接(walk)が1.6秒続いたら定位置に戻して**サイクルを繰り返す**（`animTestSamurai`/`animTestResetT`）。
