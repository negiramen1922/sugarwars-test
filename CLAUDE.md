...
- **ショートケーキ（スネイル型）**：`shortcake_blue`(ブルーベリー)/`shortcake_red`(いちご)＝イチゴを背負ったスネイル。**タレット設置中**（`u.myTurret` 生存）はイチゴを下ろした `shortcake_deploy_*` に切替（背中のベリー＝タレットになって飛び出した表現）。**戦闘中(`world.phase==='battle'`)は歩行2フレーム**＝背負い `shortcake_blue`/`shortcake2_blue`・設置中 `shortcake_deploy_blue`/`shortcake_deploy2_blue` を `SHORTCAKE_WALK_FPS`(4)で切替（**下の足の並々が波打つ**・`u.x`で個体位相ずらし）／待機・カードは frame1。地面に接地する普通のユニット描画（旧「本体だけ浮遊＋影別描画」＝`shortcake_shadow`は撤廃）。青=ブルーベリー/赤=いちご（アップ元絵はいちご＝赤基準で、青は**イチゴのピクセルだけ**＝`r-g>65`の濃い赤のみを**参考カラーの明るいコーンフラワー青(rgb87,126,235)へ輝度ランプで置換**し**クリーム色の体はそのまま**＝苺→ブルーベリー。単純なch入替でなく輝度→青ランプなので陰影が残る）。全4スネイル絵は同一bboxで64px→128pxにアンカー（背負い/下ろしで本体位置が揃う）。タレット `strawturret_blue`(ブルーベリー)/`strawturret_red`(いちご)。**アニメ動作確認**＝設定「🧪 アニメテスト」→ショートケーキを選ぶと `animTestLive` で味方青/敵赤を上下に置き `stepWorld` を回す＝前進→タレット設置→歩行/連射を実演。テスト：test.js 123。
