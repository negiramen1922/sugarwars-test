...
;globalThis.__API = {
  ...
  castSpell, castThunder, castReroll, spellRemaining, canCastSpell, spellsActive, renderSpellBar,
...
console.log('\n=== 128) 呪文（スペル）：下準備フェーズ（テスト対戦限定・プレイヤーのみ） ===');
{
  ...
  // --- ⚡雷おこし：ランダムな敵最大5体に落雷（無敵/狙えない対象は除外） ---
  API.setupCanvas();
  const W = API.CW_get() || 440, H = API.CH_get() || 760;
  let w = API.createWorld(W, H); w.phase = 'muster'; API.world = w;
  API.state.spellsOn = true;
  ...
  const nFoe = foes.length;   // choco count=2 ×4 = 8
  ...
  const hit = API.castThunder();
  check('雷おこしは最大5体（敵8体→ちょうど5体）', hit === API.SPELL_THUNDER_TARGETS);
  ...
  check('敵が5体未満なら全員に落雷', API.castThunder() === three.length);

  // --- castSpell 統合：消費タイプはピックを1つ進め回数を1消費 ---
