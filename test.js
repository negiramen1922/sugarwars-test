...
;globalThis.__API = {
  ...
  masteryOf, avatarUnlocked, bumpMasteryForDeck, get MASTERY_NEED(){ return MASTERY_NEED; },
...
console.log('\n=== 79) 熟練度で特別アバターを解禁 ===');
{
  ...
  // need-1回ではまだロック
  for (let i = 0; i < need - 1; i++) API.bumpMasteryForDeck();
  check(`(need-1)回ではまだロック（熟練度=${need - 1}）`, API.masteryOf('cookie') === need - 1 && API.avatarUnlocked('ava_cookie') === false, API.masteryOf('cookie'));
  // ちょうどneed回で解禁
  API.bumpMasteryForDeck();
  check('need回で解禁される', API.masteryOf('cookie') === need && API.avatarUnlocked('ava_cookie') === true, API.masteryOf('cookie'));
  check('デッキ外のキャラは熟練度が上がらない', API.masteryOf('daifuku') === 0);
  // 保存の往復で熟練度が保たれる
  const saved = API.buildProfile();
  check('buildProfileに熟練度が入る', saved.mastery && saved.mastery.cookie === need, saved.mastery);
  API.myProfile.mastery = {};
  API.applyProfile({ mastery: { cookie: need } });
  check('applyProfileで熟練度が復元される', API.masteryOf('cookie') === need);
  // ロック中の特別アイコンはavatarに採用されない（saveProfileの防御と同じ判定）
  API.myProfile.mastery = {};
  check('未解禁のava_bombはavatarUnlocked=false', API.avatarUnlocked('ava_bomb') === false);
}
