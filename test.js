...
console.log('\n=== 100) 通常プレイに test:true βキャラが漏れない ===');
{
  ...
  // ※クマグミは正式実装（test:true撤去）したのでこのリストから除外
  ['mangel', 'purin', 'shortcake'].forEach(k => {
  ...
  check('kumagumi は正式実装（test:trueではない）', !API.UNIT_BY_KEY['kumagumi'].test);
...
console.log('\n=== 106) エンジェル：基礎HP300／固有強化「天使の加護」で500 ===');
{
  check('エンジェルの基礎HPは300', API.UNIT_BY_KEY['mangel'].hp === 300);
  check('MANGEL_BUFF_HP は500', API.MANGEL_BUFF_HP === 500);
  ...
  check('天使の加護でHPが500に', ang.maxHp === 500 && ang.hp === 500, { hp: ang.hp });
