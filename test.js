...
console.log('\n=== 92) キャラ解禁：スターター＋💎ジェムで解禁 ===');
{
  ...
  // ジェム購入：足りないと不可
  prof.gems = API.unitUnlockCost('donut')-1;
  check('残高不足では購入不可', API.buyUnit('donut')===false && API.unitUnlocked('donut')===false);
  // 足りれば解禁＋残高が減る
  prof.gems = API.unitUnlockCost('donut')+50;
  const g0=API.myGems();
  check('購入で解禁', API.buyUnit('donut')===true && API.unitUnlocked('donut')===true);
  check('購入でジェムが減る', API.myGems()===g0-API.unitUnlockCost('donut'), API.myGems());
  ...
  // ジェム報酬（純粋関数）
  check('負けは💎0', API.battleGemReward(false,false)===0);
  check('勝ちは💎GEM_WIN', API.battleGemReward(true,false)===API.GEM_WIN);
  check('勝ち＋初勝利は💎WIN+DAILY', API.battleGemReward(true,true)===API.GEM_WIN+API.GEM_DAILY);
  // grantBattleRewardが💎も返す/加算する
  prof.gems=0; prof.coins=0; prof.dailyWinKey='';
  const gr=API.grantBattleReward(true);
  check('grantBattleReward: gems=WIN+DAILY(初勝利)', gr.gems===API.GEM_WIN+API.GEM_DAILY, gr);
  check('grantBattleReward: 💎が加算', API.myGems()===API.GEM_WIN+API.GEM_DAILY);
  // 保存往復
  prof.units=['donut','pancake']; prof.gems=250; prof.guideDone=['front'];
  const p=API.buildProfile();
  check('buildProfile: gems/units/guideDone', p.gems===250 && p.units.includes('donut') && p.guideDone.includes('front'));
  prof.units=['donut']; prof.gems=100; prof.guideDone=[];
  API.applyProfile({ gems:500, units:['ghost','cookie'], guideDone:['front','ranged'] });
  check('applyProfile: gemsは大きい方(500)', prof.gems===500, prof.gems);
