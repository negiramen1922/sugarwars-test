...
console.log('\n=== 129) 新キャラ：エクレアドッグ＆ワッフルランサー（開発テスト中） ===');
{
  ...
  check('wafflelancer は charger + pierce=2（貫通2＝大群に埋もれる調整）', lancer.charger === true && lancer.pierce === 2);
    ...
    ['wafflelancer_body_blue','wafflelancer_body_red','wafflelancer_lance'].every(k => !!API.SPRITE_DATA[k]));
  {
    ...
    check('fighter に pierce がコピーされる', lf.pierce === 2);
...

  {
    ...
    const start = foes.map(f => f.hp);
    let frames = 0; while (frames < 60 * 4 && !wl.done) { API.stepWorld(wl, 1 / 60); frames++; }
    const damaged = foes.filter((f, i) => f.hp < start[i]).length;
    check('突撃が複数の敵を貫く（2体以上に突撃ダメージ）', damaged >= 2, { damaged });
  ...
  check('wafflelancer: atk26 / hp220 / dashDamage100 / pierce2（相性調整）', lancer.atk === 26 && lancer.hp === 220 && lancer.dashDamage === 100 && lancer.pierce === 2,
