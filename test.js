...
;globalThis.__API = {
  ...
  pvpInterpFrame, get PVP_SNAP_DELAY(){ return PVP_SNAP_DELAY; }, get PVP_MAX_EXTRAP(){ return PVP_MAX_EXTRAP; }, get PVP_JUMP_EPS(){ return PVP_JUMP_EPS; },
...
console.log('\n=== 128) PVP：子側スナップショット補間＋枯渇時の外挿（ラグ差の緩和） ===');
{
  const D = API.PVP_SNAP_DELAY, MX = API.PVP_MAX_EXTRAP, JE = API.PVP_JUMP_EPS;
  const mkw = (units) => ({ W:440, H:760, phase:'battle', units, parts:[], rings:[], slashes:[], dmgs:[], puddles:[] });
  const u = (nid, x, y) => ({ nid, x, y, side:'p', hp:50, bob:0 });
  // バッファ：t=0 に x=100、t=100 に x=160（1フレーム60px移動＜JE=90 なので補間対象。速度 0.6px/ms）
  const V = 0.6, X0 = 100, X1 = X0 + V*100;   // =160
  const buf = [ { t:0, w:mkw([u(1,X0,300)]) }, { t:100, w:mkw([u(1,X1,300)]) } ];

  // 空/1点のフォールバック
  check('補間：空バッファ→null', API.pvpInterpFrame([], 1000, D, MX) === null);
  check('補間：1点のみ→その盤面', API.pvpInterpFrame([buf[0]], 1000, D, MX).units[0].x === X0);

  // now=100+D → rt=100 → ちょうど最新点（線形補間の端）でx=X1
  const atLatest = API.pvpInterpFrame(buf, 100 + D, D, MX);
  check('補間：rtが最新点ちょうど→最新座標', Math.abs(atLatest.units[0].x - X1) < 0.5);

  // now=50+D → rt=50 → 2点の中点でx=X0+V*50
  const mid = API.pvpInterpFrame(buf, 50 + D, D, MX);
  check('補間：2点の中点→中間座標', Math.abs(mid.units[0].x - (X0 + V*50)) < 0.5);

  // now=(-100)+D → rt=-100 → 過去すぎ→最古(x=X0・外挿しない)
  const tooOld = API.pvpInterpFrame(buf, -100 + D, D, MX);
  check('補間：過去すぎ→最古で止める', Math.abs(tooOld.units[0].x - X0) < 0.5);

  // 枯渇時の外挿：rt=最新t+50（未来側なし）→ 速度V×50ms 先読み
  const ext = API.pvpInterpFrame(buf, 150 + D, D, MX);
  check('外挿：枯渇時は速度で先読み', Math.abs(ext.units[0].x - (X1 + V*50)) < 0.5);

  // 外挿の上限：rt=最新t+500 でも maxExtrap(120ms) で頭打ち→x=X1+V*MX
  const extCap = API.pvpInterpFrame(buf, 600 + D, D, MX);
  check('外挿：maxExtrapで頭打ち（暴走しない）', Math.abs(extCap.units[0].x - (X1 + V*MX)) < 0.5);

  // ワープ（1フレームで JE 超の移動）は補間しない＝最新へ素通し
  const warp = [ { t:0, w:mkw([u(2,100,300)]) }, { t:100, w:mkw([u(2,100 + JE + 50,300)]) } ];
  const wmid = API.pvpInterpFrame(warp, 50 + D, D, MX);
  check('補間：ワープ(大ジャンプ)は補間せず最新へ素通し', wmid.units[0].x === 100 + JE + 50);

  // 定数の妥当性
  check('定数：DELAY>0 かつ maxExtrap>0', D > 0 && MX > 0 && JE > 0);
}
