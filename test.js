...
;globalThis.__API = {
  ...
  evolvePancake, evolvedStep, nearestEnemy, spawnerStep, BAKERY_SPAWN_PATTERN, chargerStep,
  ...
  realDeck, setGuideBackup:(d)=>{ _guideDeckBackup = d; }, clearGuideBackup:()=>{ _guideDeckBackup = null; },
  ...
  spriteFor, get SPRITES(){ return SPRITES; }, animTestUnits, animTestValid, slimeHopPhase, slimeHopLift, SLIME_HOP_FPS:()=>SLIME_HOP_FPS, BOMB_WALK_FPS:()=>BOMB_WALK_FPS, MAC_PAKA_FPS:()=>MAC_PAKA_FPS, SODA_WALK_FPS:()=>SODA_WALK_FPS, SHORTCAKE_WALK_FPS:()=>SHORTCAKE_WALK_FPS,
  ...
  get SPELLS(){ return SPELLS; }, get SPELL_ORDER(){ return SPELL_ORDER; },
  get SPELL_THUNDER_TARGETS(){ return SPELL_THUNDER_TARGETS; }, get SPELL_THUNDER_DMG(){ return SPELL_THUNDER_DMG; },
  castSpell, castThunder, castReroll, fireThunderStrikes, spawnThunderBolt, spellRemaining, spellUsedThisRound, canCastSpell, spellsActive, renderSpellBar,
  setSpellsTest:(v)=>{ _spellsTest = v; },
};
`;
...
API.state.noX2 = false;   // X2機構そのものの検証（β1.5でX2は既定オフ・機構は残存）
...
console.log('\n=== 19) 大福サムライ：突撃＋通常攻撃とも前方範囲(cleaveNormal・ランサーと差別化)＋固有強化「特大大福」 ===');
// (A) デフォルト：突撃も通常攻撃も前方の複数の敵をまとめて斬る（cleaveNormal）
...
// 通常攻撃も前方範囲(cleaveNormal)：前方に並ぶ複数の敵にまとめて当たる（数フレーム）
...
check('通常攻撃も前方範囲＝複数に当たる（cleaveNormal）', walkCleaveHits() >= 2);
...
check('強炭酸で沼のダメージが上がる（5→7）', ps.puddleDps === API.SODA_BUFF_DPS && ps.puddleDps > sBase.puddleDps, { base: sBase.puddleDps, now: ps.puddleDps });
check('強炭酸で爆発威力が上がる（10→20）', ps.blast === API.SODA_BUFF_BLAST && ps.blast > sBase.blast, { base: sBase.blast, now: ps.blast });
...
function sodaPuddle(fizz) {
  ...
  wd.units.push(s);
  if (fizz) API.applySodaFizz(wd, 'p');   // 実際の強化処理を通す（範囲拡大＋威力/沼ダメージUP）
  API.killUnit(wd, s);
  return wd.puddles[0];
}
...
check('強炭酸で沼のDPSが上がる（5→7）', fizzPud.dps > basePud.dps && fizzPud.dps === API.SODA_BUFF_DPS, { base: basePud.dps, fizz: fizzPud.dps });
...
API.state.noX2 = false;   // X2機構の検証（既定オフでも機構は残す）
...
console.log('\n=== 52) 炭酸沼の重ねがけ上限（PUDDLE_DPS_CAP） ===');
{
  ...
  wd.units.push(e);
  for (let i = 0; i < 6; i++) wd.puddles.push({ x: 220, y: 330, r: 90, life: 9, maxLife: 9, side: 'p', dps: 5, slow: 0.5, bub: 0 });  // dps5×6=合計30/秒
  const before = e.hp;
  API.stepWorld(wd, 1.0);   // 1秒進める
  const lost = before - e.hp;
  check('重ねがけは上限/秒で頭打ち（合計30でもCAP=15前後）', lost > 14 && lost < 16, { lost });
  check('上限なしの合計(30)より小さい', lost < 29, { lost });
}
...
console.log('\n=== 57) X2上限: 多すぎる種類(>=X2_OFFER_CAP)にはX2カードを出さない ===');
{
  ...
  API.state.noX2 = false;   // X2上限ロジックの検証（既定オフでも機構は残す）
...
console.log('\n=== 64.5) ドーナッツ調整: 高ノックバック耐性＋CPU評価の見直し ===');
{
  // kbResist フラグが立つ
  const d0 = API.makeFighters('donut', 'p', 440, 660, 'army')[0];
  ...
  const kbTest = (key) => {
    const wd = API.createWorld(440, 660); API.world = wd;
    const t = API.makeFighters(key, 'p', 440, 660, 'army')[0]; t.x = 220; t.y = 300; t.kbImmune = 0; wd.units.push(t);
    const a = API.makeFighters('cookie', 'e', 440, 660, 'army')[0]; a.x = 220; a.y = 320; wd.units.push(a);
    API.applyHit(wd, a, t, 10);
    return Math.hypot(t.knockx, t.knocky);
  };
  check('ドーナッツは殴られてもノックバックしない', kbTest('donut') === 0);
  check('通常ユニット(cookie)はノックバックする', kbTest('cookie') > 0);

  // aiPicks: 高HP偏重をやめたので、火力役(cookie)を吸引タンク(donut)より優先する
  let cookieWins = 0;
  for (let i = 0; i < 200; i++) { const p = API.aiPicks(['donut', 'cookie'], 1)[0]; if (p === 'cookie') cookieWins++; }
  check('CPUはdonutより火力役cookieを優先(>150/200)', cookieWins > 150, { cookieWins });
  // ただし弱いユニットよりはドーナッツを選ぶ（無価値化はしない）
  let donutOverGinger = 0;
  for (let i = 0; i < 200; i++) { const p = API.aiPicks(['donut', 'ginger'], 1)[0]; if (p === 'donut') donutOverGinger++; }
  check('CPUは弱ユニットよりdonutを選ぶ(>150/200)', donutOverGinger > 150, { donutOverGinger });
}
...
console.log('\n=== 77) 新キャラ シェルマカロン（殻スピン→スタン→通常／ビッグシェル強化） ===');
{
  ...
  check('マカロンアーマーでHPが増える(100→180)', m2.maxHp === Math.round(baseHp * (1 + 0.8)) && m2.maxHp === 180 && m2.hp === m2.maxHp, { base: baseHp, now: m2.maxHp });
  ...
  API.applyMacaronBuff(wb, 'p');
  check('2回適用しても重ねがけしない', m2.maxHp === Math.round(baseHp * (1 + 0.8)));
  // 詳細表示にマカロンアーマーが出る（HP増加なのでstatsあり）
  const ed = API.enhDisplay(mc);
  const be = ed.find(x => x.kind === '強化');
  check('詳細にマカロンアーマーが出てHPが併記される', be && be.name === 'マカロンアーマー' && be.stats && be.stats.hp > mc.hp, be && be.stats);
}
...
{
  // チョコ：強化（ビター装甲）の強化後ステータスが出る（HP200→280・atk20→30）
  const ec = API.enhDisplay(API.UNIT_BY_KEY['choco']);
  const buff = ec.find(e => e.kind === '強化');
  check('チョコに強化エントリが出る', !!buff, ec.map(e => e.kind));
  check('強化後HP=280', buff && buff.stats && buff.stats.hp === 280, buff && buff.stats);
  check('強化後ATK=30', buff && buff.stats && buff.stats.atk === 30, buff && buff.stats);
...
{
  const prof = API.myProfileRef;
  prof.mastery={}; prof.collected={}; prof.coins=0; prof.frame=''; prof.titlePre=''; prof.titleSuf=''; prof.nameColor=''; prof.dailyWinKey='';
  ...
  // 重複：dup・テーマキャラ(frame_gold=pancake)の熟練度XPに変換
  ...
  check('重複：テーマキャラ(pancake)に+PACK_DUP_XP', res2.xp===API.PACK_DUP_XP && API.masteryXp('pancake')===API.PACK_DUP_XP, {xp:res2.xp, my:API.masteryXp('pancake')});
  check('重複でコインは増えない（重複報酬は熟練度XP）', API.myCoins()===0, API.myCoins());
  // avatarUnlockedは熟練度Lvで判定（アイコンはガチャ非排出＝collectedには基本入らないが、fallbackは維持）
  prof.mastery={}; prof.collected={};
  check('低熟練度アバターは未解禁', API.avatarUnlocked('ava_choco')===false);
...
console.log('\n=== 91) 経済：プロフィール保存の往復（コイン/コレクション/装備） ===');
{
  ...
  prof.coins=0; prof.collected={}; prof.frame=''; prof.titlePre=''; prof.titleSuf=''; prof.nameColor=''; prof.dailyWinKey='';
}

console.log('\n=== 92) キャラ解禁：スターター＋🍬シュガーコインで解禁 ===');
{
  const prof = API.myProfileRef;
  prof.units=[]; prof.coins=0; prof.guideDone=[];
  API.setMyDeck([]);
  // スターターは最初から解禁
  check('cookieはスターター解禁', API.unitUnlocked('cookie')===true);
  check('chocoはスターター解禁', API.unitUnlocked('choco')===true);
  check('shoeはスターター解禁', API.unitUnlocked('shoe')===true);
  // 非スターターはロック
  check('donutはロック', API.unitUnlocked('donut')===false);
  check('pancakeはロック', API.unitUnlocked('pancake')===false);
  check('召喚専用gingerは解禁対象外(false)', API.unitUnlocked('ginger')===false);
  // ロック一覧にスターターは含まれない
  const lk=API.lockedUnits();
  check('lockedUnitsにdonut含む', lk.includes('donut'));
  check('lockedUnitsにcookie含まない', !lk.includes('cookie'));
  check('lockedUnitsにginger含まない（召喚専用）', !lk.includes('ginger'));
  // シュガーコイン購入：足りないと不可
  prof.coins = API.unitUnlockCost('donut')-1;
  check('残高不足では購入不可', API.buyUnit('donut')===false && API.unitUnlocked('donut')===false);
  // 足りれば解禁＋残高が減る
  prof.coins = API.unitUnlockCost('donut')+50;
  const g0=API.myCoins();
  check('購入で解禁', API.buyUnit('donut')===true && API.unitUnlocked('donut')===true);
  check('購入でシュガーコインが減る', API.myCoins()===g0-API.unitUnlockCost('donut'), API.myCoins());
  check('解禁済みは再購入不可', API.buyUnit('donut')===false);
  // 既存デッキの救済（migrate）：デッキ内の非スターターは解禁扱いに
  prof.units=[]; API.setMyDeck(['choco','pancake','ghost','cookie']);
  API.migrateUnlocks();
  check('migrate: pancakeが解禁される', API.unitUnlocked('pancake')===true);
  check('migrate: ghostが解禁される', API.unitUnlocked('ghost')===true);
  check('migrate: スターターは重複登録しない', prof.units.indexOf('cookie')<0);
  // 保存往復（gemsは廃止＝coinsのみ）
  prof.units=['donut','pancake']; prof.coins=250; prof.guideDone=['front'];
  const p=API.buildProfile();
  check('buildProfile: coins/units/guideDone', p.coins===250 && p.units.includes('donut') && p.guideDone.includes('front'));
  check('buildProfile: gemsは書き出さない（廃止）', p.gems===undefined, p.gems);
  prof.units=['donut']; prof.coins=100; prof.guideDone=[];
  API.applyProfile({ coins:500, units:['ghost','cookie'], guideDone:['front','ranged'] });
  check('applyProfile: coinsは大きい方(500)', prof.coins===500, prof.coins);
  check('applyProfile: unitsは和集合', prof.units.includes('donut')&&prof.units.includes('ghost'));
  check('applyProfile: スターターはunitsに入れない', prof.units.indexOf('cookie')<0);
  check('applyProfile: guideDoneは和集合', prof.guideDone.includes('front')&&prof.guideDone.includes('ranged'));
  prof.units=[]; prof.coins=0; prof.guideDone=[]; API.setMyDeck([]);
}

console.log('\n=== 93) PVE戦術指南：順番制＋クリアで解禁＋ジェム ===');
{
  const prof = API.myProfileRef;
  prof.units=[]; prof.coins=0; prof.guideDone=[];
  ...
  check('別アーク先頭も最初から挑戦可（ranged1）', API.guideStageUnlocked('ranged1')===true);
  ...
  // frontアークを勝利クリア
  prof.coins=0;
  const front=API.GUIDE_STAGES.find(s=>s.id==='front'), g0=API.myCoins();
  API.setGuideStage('front');
  const line=API.guideFinish(true);
  check('クリアで報酬キャラ解禁(donut)', API.unitUnlocked('donut')===true);
  check('クリアで🍬付与', API.myCoins()===g0+front.coins, API.myCoins());
  ...
  // 2回目クリアはコイン二重取りしない
  const g1=API.myCoins();
  API.setGuideStage('front'); API.guideFinish(true);
  check('クリア済みは🍬二重取りしない', API.myCoins()===g1, API.myCoins());
  // 負けたら解禁しない
  prof.units=[]; prof.guideDone=[];
  API.setGuideStage('ranged2');
  API.guideFinish(false);
  check('負けでは解禁されない(icewiz)', API.unitUnlocked('icewiz')===false);
  check('負けではguideDoneに入らない', API.guideCleared('ranged2')===false);
  prof.units=[]; prof.coins=0; prof.guideDone=[]; API.setMyDeck([]);
}
...
console.log('\n=== 95) アニメ付きキャラ（マシュマロエンジェル）：フレーム切替＋テスト除外 ===');
{
  ...
  check('正式実装（test:trueではない）', U && !U.test);
  ...
  // ランダムCPUデッキ（召喚専用・test を除外した通常ロスター）にテストキャラは入らない設計。
  // β1.6 でカヌレ/スネイルが正式実装され、現在 test:true βは0体でも「除外設計」は成立する。
  const normalPool = API.UNITS.filter(u => !u.summonOnly && !u.test).map(u => u.key);
  check('通常ロスターに test:true キャラは混ざらない（除外設計）', API.UNITS.filter(u => u.test).every(u => !normalPool.includes(u.key)));
  // クマグミ（正式実装済み・静止・陣営色を1枚アニメで出し分け）
  const K = API.UNIT_BY_KEY.kumagumi;
  check('kumagumi が存在し正式実装（test:trueではない）', !!K && !K.test);
  ...
  // アニメテストで選択できるキャラ（test:true ＋ ANIM_TEST_EXTRA）が1体以上あること。
  // test:true βが0体でも ANIM_TEST_EXTRA（strawturret/daifuku/cookie/choco）で成立する。
  check('アニメテストで選択できるキャラが1体以上', API.animTestUnits().length >= 1, API.animTestUnits().map(u => u.key));
}
...
console.log('\n=== 98) クマグミ: 倒した敵を自軍のクマグミに仲間化 ===');
{
  ...
  check('子グミは本体より弱い（HP/攻撃が専用値）', spawned && spawned.hp === 30 && spawned.maxHp === 30 && spawned.atk === 6, spawned && { hp: spawned.hp, atk: spawned.atk });
  check('本体は count4・atk8・hp50', API.UNIT_BY_KEY['kumagumi'].count === 4 && API.UNIT_BY_KEY['kumagumi'].atk === 8 && API.UNIT_BY_KEY['kumagumi'].hp === 50);
...
console.log('\n=== 99) マシュマロエンジェル: 浮遊（近接では狙えない）＋肩代わり ===');
{
  ...
  const passExpect = Math.max(0, 20 - soakExpect);
  ...
  check('100%肩代わりなら味方は無傷（範囲内・近接/遠距離）', API.MANGEL_SOAK < 1 || ally.hp === allyHp0, { soak: API.MANGEL_SOAK, ally: ally.hp });
  ...
  // 範囲攻撃(爆発AoE)も肩代わりする：ボム爆発で範囲内の味方が受けるダメージをエンジェルが背負う
  const wd5 = API.createWorld(W, H); wd5.phase = 'battle'; wd5.intro = 0; API.world = wd5;
  const g = API.makeFighters('mangel', 'p', W, H, 'army')[0]; g.appear = 1; g.x = 300; g.y = 300; g.hp = g.maxHp; wd5.units.push(g);
  const prot = API.makeFighters('cookie', 'p', W, H, 'army')[0]; prot.appear = 1; prot.x = 305; prot.y = 300; prot.hp = prot.maxHp; wd5.units.push(prot);
  const bomb = API.makeFighters('bomb', 'e', W, H, 'army')[0]; bomb.appear = 1; bomb.x = 305; bomb.y = 300; wd5.units.push(bomb);
  const gHp0 = g.hp, protHp0 = prot.hp;
  API.killUnit(wd5, bomb);   // ボムを起爆＝範囲内の味方(cookie)に爆発ダメージ
  check('AoE(爆発)で守られた味方は無傷（100%肩代わり）', prot.hp === protHp0, { prot: prot.hp });
  check('AoE(爆発)ぶんをエンジェルが背負う（HP減少）', g.hp < gHp0, { g: g.hp, was: gHp0 });
}

console.log('\n=== 99b) マシュマロエンジェル：攻撃せず味方に追尾する ===');
{
  const wd = API.createWorld(W, H); wd.phase = 'battle'; wd.intro = 0; API.world = wd;
  const ang = API.makeFighters('mangel', 'p', W, H, 'army')[0]; ang.appear = 1; ang.x = 100; ang.y = 650; wd.units.push(ang);
  // 守る味方を離れた場所に置く → エンジェルはそちらへ寄っていくはず（敵は真逆に置く）
  const ally = API.makeFighters('cookie', 'p', W, H, 'army')[0]; ally.appear = 1; ally.x = 380; ally.y = 620; wd.units.push(ally);
  const en = API.makeFighters('choco', 'e', W, H, 'army')[0]; en.appear = 1; en.x = 100; en.y = 120; en.hp = en.maxHp = 9999; wd.units.push(en);
  const d0 = Math.hypot(ally.x - ang.x, ally.y - ang.y);
  for (let f = 0; f < 60; f++) API.stepWorld(wd, 1 / 60);
  const d1 = Math.hypot(ally.x - ang.x, ally.y - ang.y);
  check('エンジェルは守る味方に近づく（追尾）', d1 < d0, { d0: Math.round(d0), d1: Math.round(d1) });
  // 敵の方（上）へは行かない＝敵に突っ込まない（yがほぼ下のまま）
  check('エンジェルは敵に突っ込まない（味方側に留まる）', ang.y > 400, { y: Math.round(ang.y) });
}

console.log('\n=== 100) 通常プレイに test:true βキャラが漏れない ===');
{
  // 開発中(test:true)のβキャラは、ランダム相手デッキ・スターター・X2生成に混ざらないこと
  // ※クマグミ/マシュマロエンジェル/プリン、そして β1.6 でカヌレ/スネイルも正式実装（test:true撤去）
  //   → 現在 test:true の通常非登場βキャラは0体。正式実装組は test:true でないこと＆X2生成されることを確認。
  ['kumagumi', 'mangel', 'purin', 'canule', 'shortcake'].forEach(k => {
    check(k + ' は正式実装（test:trueではない）', !API.UNIT_BY_KEY[k].test);
  });
  check('x2_shortcake は自動生成される（正式実装後）', !!API.SPECIALS['x2_shortcake']);
  check('strawturret は summonOnly（ロスター/ピッカー非対象）', API.UNIT_BY_KEY['strawturret'].summonOnly === true);
}

console.log('\n=== 101) プリン: ブーメラン（往復で経路上の敵を薙ぐ） ===');
{
  const wd = API.createWorld(W, H); API.world = wd;
  const purin = API.makeFighters('purin', 'p', W, H, 'army')[0]; purin.appear = 1; purin.x = 100; purin.y = 300;
  wd.units.push(purin);
  check('boomerangフラグを持つ', purin.boomerang === true);
  // 右方向に敵をおいてブーメランを投げる
  const foe = API.makeFighters('choco', 'e', W, H, 'army')[0]; foe.appear = 1; foe.x = 200; foe.y = 300; foe.hp = 400;
  wd.units.push(foe);
  API.throwBoomerang(wd, purin, foe);
  check('ブーメランが飛んでいる', wd.shots.length === 1 && wd.shots[0].boom === true);
  const boom = wd.shots[0];
  const hp0 = foe.hp;
  // 往路：ブーメランが敵の位置に到達するまで進める → 1回ヒット
  for (let i = 0; i < 120 && boom.out; i++) API.stepBoomerang(wd, boom, 1 / 60);
  check('往路で敵に当たった', foe.hp < hp0, { hp0, now: foe.hp });
  const hpAfterOut = foe.hp;
  check('往路の折り返しに入った（out=false）', boom.out === false);
  check('折り返しでヒット記録がリセットされ敵は未ヒット扱い', !boom.hit.has(foe));
  // 復路：手元に戻るまで進める → 帰りにもう1回ヒット、最後に消える
  for (let i = 0; i < 240 && !boom.done; i++) API.stepBoomerang(wd, boom, 1 / 60);
  check('復路でもう一度当たった（往復2ヒット）', foe.hp < hpAfterOut, { out: hpAfterOut, back: foe.hp });
  check('手元に戻って消える', boom.done === true);
  // 味方には当たらない
  const wd2 = API.createWorld(W, H); API.world = wd2;
  const p2 = API.makeFighters('purin', 'p', W, H, 'army')[0]; p2.appear = 1; p2.x = 100; p2.y = 300; wd2.units.push(p2);
  const ally = API.makeFighters('cookie', 'p', W, H, 'army')[0]; ally.appear = 1; ally.x = 160; ally.y = 300; ally.hp = ally.maxHp; wd2.units.push(ally);
  API.throwBoomerang(wd2, p2, { x: 250, y: 300 });
  for (let i = 0; i < 300 && !wd2.shots[0].done; i++) API.stepBoomerang(wd2, wd2.shots[0], 1 / 60);
...
console.log('\n=== 103) プリン/ショートケーキ 実戦スモーク（stepWorldで例外なく回る） ===');
{
  ...
  try {
    for (let f = 0; f < 600; f++) {
      API.stepWorld(wd, 1 / 60);
      if (wd.units.some(u => u.key === 'strawturret')) turretSeen = true;
      if (wd.shots.some(s => s.boom)) boomSeen = true;
      if (wd.phase === 'outro') break;
    }
  } catch (e) { threw = true; console.log('    err:', e && e.message); }
  check('600フレーム回して例外が出ない', threw === false);
  check('開幕にタレットが設置された', turretSeen === true);
...
console.log('\n=== 106) エンジェル：基礎HP300／固有強化「アークエンジェル」でHP+60% ===');
{
  check('エンジェルの基礎HPは300', API.UNIT_BY_KEY['mangel'].hp === 300);
  check('MANGEL_BUFF_HP は480', API.MANGEL_BUFF_HP === 480);
  check('buff_mangel が SPECIALS に存在', !!API.SPECIALS['buff_mangel'] && API.SPECIALS['buff_mangel'].mangelBuff === true);
  const wd = API.createWorld(W, H); API.world = wd;
  const ang = API.makeFighters('mangel', 'p', W, H, 'army')[0]; ang.appear = 1; ang.x = 200; ang.y = 300; wd.units.push(ang);
  check('強化前のHPは300', ang.maxHp === 300 && ang.hp === 300);
  API.applyMangelBuff(wd, 'p');
  check('アークエンジェルでHPが480に', ang.maxHp === 480 && ang.hp === 480, { hp: ang.hp });
  check('強化フラグが立つ', ang.mangelBuff === true);
  // 提示候補：エンジェルが居れば buff_mangel が出る（未取得時）
  API.resetState();
  const wd2 = API.createWorld(W, H); API.world = wd2;
  API.makeFighters('mangel', 'p', W, H, 'army').forEach(f => { f.appear = 1; wd2.units.push(f); });
  check('エンジェルが居れば buff_mangel が提示候補に入る', API.eligibleSpecials().includes('buff_mangel'));
}

console.log('\n=== 107) テスト対戦モード：X2(2倍)カードを無効化 ===');
{
  // X2が出る状況（同キャラ3体以上）を作る
  API.resetState();
  const wd = API.createWorld(W, H); API.world = wd;
  API.makeFighters('cookie', 'p', W, H, 'army').forEach(f => { f.appear = 1; wd.units.push(f); });   // cookieはcount5＝X2_MIN(3)以上・X2_OFFER_CAP(15)未満
  API.state.noX2 = false;
  check('通常時はX2候補が出る（cookie5体）', API.eligibleX2Specials().length > 0, API.eligibleX2Specials());
  API.state.noX2 = true;
  check('noX2モードではX2候補が空', API.eligibleX2Specials().length === 0);
  check('noX2モードでは eligibleSpecials にもX2が含まれない', !API.eligibleSpecials().some(k => k.indexOf('x2_') === 0));
  // 敵側もX2を使わない
  API.makeFighters('choco', 'e', W, H, 'army').forEach(f => { f.appear = 1; wd.units.push(f); });
  API.makeFighters('choco', 'e', W, H, 'army').forEach(f => { f.appear = 1; wd.units.push(f); });   // choco count2×2=4体（X2_MIN以上）
...
console.log('\n=== 108) クマグミ固有強化「コグマの成長」：仲間化コグマが親と同じに ===');
{
  ...
  check('本体も強化される（HP60/攻撃9）', kuma2.maxHp === 60 && kuma2.atk === 9, { hp: kuma2.maxHp, atk: kuma2.atk });
  ...
  check('強化後コグマは強化後の親と同じ(hp60/atk9)', child2 && child2.hp === 60 && child2.atk === 9, child2 && { hp: child2.hp, atk: child2.atk });
  check('強化コグマも強化マーカーを継ぐ', child2 && child2.kumaBuff === true);
  API.state.youKumaBuff = false;
}

console.log('\n=== 110) ロック解放モーダル：戦術指南への導線（guideArcForUnit） ===');
{
  // 全てのロック対象（非スターターのデッキ可能キャラ）に、そのキャラを解放する戦術指南アークが対応する
  const starters = new Set(API.STARTER_UNITS);
  const nonStarter = API.UNITS.filter(u => !u.summonOnly && !u.test && !starters.has(u.key)).map(u => u.key);
  const missing = nonStarter.filter(k => !API.guideArcForUnit(k));
  check('全ロックキャラに解放アークが対応する（戦術指南ボタンの遷移先が必ずある）', missing.length === 0, missing);
  const dArc = API.guideArcForUnit('donut');
  check('donutに解放アークがある', !!dArc, dArc);
  check('その解放アークにdonut解禁レッスンが含まれる', API.GUIDE_STAGES.some(s => (s.arc || s.unit || s.id) === dArc && s.unit === 'donut'));
  check('スターター(cookie)は解放アークなし=null', API.guideArcForUnit('cookie') === null);
}

console.log('\n=== 107) 戦術指南：クッキーじいのキャラ紹介＋アイス2レッスン ===');
{
  // 学ぶ全キャラ（アークが解禁する非スターター）に紹介セリフがある
  const arcs = API.GUIDE_STAGES.map(s => (s.arc || s.unit || s.id));
  const learnedUnits = [...new Set(API.GUIDE_STAGES.map(s => s.forceUnit || s.unit).filter(Boolean))];
  const missing = learnedUnits.filter(k => !API.GUIDE_INTRO[k]);
  check('学ぶ全キャラにクッキーじいの紹介セリフがある', missing.length === 0, missing);
  check('紹介セリフはユニットkeyのみ（不明キーなし）', Object.keys(API.GUIDE_INTRO).every(k => !!API.UNIT_BY_KEY[k]));
  // アイスは2レッスン（ranged1=体験→ranged2=実戦でicewiz解禁）
  const ice = API.GUIDE_STAGES.filter(s => (s.arc || s.unit || s.id) === 'icewiz');
  check('アイスのアークは2レッスン', ice.length === 2, ice.map(s => s.id));
  check('ranged1は体験（unit無し・easyFoe・winRounds:1）', API.GUIDE_STAGES.find(s => s.id === 'ranged1') && !API.GUIDE_STAGES.find(s => s.id === 'ranged1').unit && API.GUIDE_STAGES.find(s => s.id === 'ranged1').easyFoe === true);
  check('ranged2でicewiz解禁', API.GUIDE_STAGES.find(s => s.id === 'ranged2') && API.GUIDE_STAGES.find(s => s.id === 'ranged2').unit === 'icewiz');
  check('ranged2はranged1クリアまでロック', API.guideStageUnlocked('ranged2') === false);
  check('報酬統一は維持（解禁=100／体験=50）', API.GUIDE_STAGES.every(s => s.unit ? s.coins === 100 : s.coins === 50));
}

console.log('\n=== 109) 指南レッスン中のデッキで誤解禁しない（migrate/保存は本デッキを使う） ===');
{
  const prof = API.myProfileRef;
  prof.units = [];
  API.clearGuideBackup();
  // レッスン中：myDeck が一時的にレッスン用デッキ（未解禁の学ぶキャラ icewiz を含む）／本デッキは退避
  API.setMyDeck(['icewiz', 'choco', 'cookie']);
  API.setGuideBackup(['cookie', 'choco', 'shoe']);
  check('realDeck はレッスン中は本デッキ（退避）を返す', JSON.stringify(API.realDeck()) === JSON.stringify(['cookie', 'choco', 'shoe']));
  API.migrateUnlocks();
  check('レッスン中の学ぶキャラ(icewiz)はmigrateで解禁されない', API.unitUnlocked('icewiz') === false, prof.units);
  check('buildProfile.deck は本デッキ（レッスン用ではない）', JSON.stringify(API.buildProfile().deck) === JSON.stringify(['cookie', 'choco', 'shoe']));
  // レッスン終了後（退避解除）：本デッキの非スターターは従来どおり救済される
  API.clearGuideBackup();
  API.setMyDeck(['icewiz', 'choco', 'cookie']);
  API.migrateUnlocks();
...
console.log('\n=== 110) プリン固有強化「ダブルスプーン」：2本投げ＋頻度ダウン＋投擲中は手ぶら ===');
{
  check('buff_purin が SPECIALS に存在', !!API.SPECIALS['buff_purin'] && API.SPECIALS['buff_purin'].purinBuff === true);
  // 提示候補
  API.resetState();
  const w0 = API.createWorld(W, H); API.world = w0;
  API.makeFighters('purin', 'p', W, H, 'army').forEach(f => { f.appear = 1; w0.units.push(f); });
  check('プリンが居れば buff_purin が提示候補に入る', API.eligibleSpecials().includes('buff_purin'));

  // 通常：1回の攻撃でスプーン1本
  const wd = API.createWorld(W, H); wd.phase = 'battle'; wd.intro = 0; API.world = wd;
  const p1 = API.makeFighters('purin', 'p', W, H, 'army')[0]; p1.appear = 1; p1.x = 200; p1.y = 500; p1.cool = 0; wd.units.push(p1);
  const t1 = API.makeFighters('choco', 'e', W, H, 'army')[0]; t1.appear = 1; t1.x = 200; t1.y = 400; t1.hp = t1.maxHp = 99999; wd.units.push(t1);
  API.stepWorld(wd, 1 / 60);
  check('通常は1回で1本', wd.shots.filter(s => s.boom).length === 1);
  check('投げてる間は手ぶら(spoonOut)', p1.spoonOut === true);

  // 強化：1回の攻撃でスプーン2本＋cdが長い
  const wd2 = API.createWorld(W, H); wd2.phase = 'battle'; wd2.intro = 0; API.world = wd2;
  const p2 = API.makeFighters('purin', 'p', W, H, 'army')[0]; p2.appear = 1; p2.x = 200; p2.y = 500; p2.cool = 0; wd2.units.push(p2);
  API.applyPurinBuff(wd2, 'p');
  check('applyPurinBuffで強化フラグ', p2.purinBuff === true);
  const t2 = API.makeFighters('choco', 'e', W, H, 'army')[0]; t2.appear = 1; t2.x = 200; t2.y = 400; t2.hp = t2.maxHp = 99999; wd2.units.push(t2);
  API.stepWorld(wd2, 1 / 60);
  check('強化は1回で2本', wd2.shots.filter(s => s.boom).length === 2);
  check('強化はcdが長い(PURIN_BUFF_CD)', Math.abs(p2.cool - API.PURIN_BUFF_CD) < 0.05, { cool: p2.cool, exp: API.PURIN_BUFF_CD });
  check('攻撃頻度ダウン（通常cdより長い）', API.PURIN_BUFF_CD > API.UNIT_BY_KEY['purin'].cd);
  // 2本は「同じ正面方向」で「発射位置だけ左右ずらし」の平行レーン（角度扇状だと遠い正面の敵に両方外れるため）
  const booms = wd2.shots.filter(s => s.boom);
  check('2本とも同じ正面方向を向く', Math.abs(booms[0].dx - booms[1].dx) < 1e-6 && Math.abs(booms[0].dy - booms[1].dy) < 1e-6);
  check('2本は発射位置が左右にずれる（平行）', Math.abs(booms[0].x - booms[1].x) > 1e-3 || Math.abs(booms[0].y - booms[1].y) > 1e-3);
  // 正面の敵（t2＝ちょうど真上）にダブルスプーンが往路で当たる（旧扇状では両方外れていた）
  const hp0 = t2.hp;
  for (let f = 0; f < 120 && booms.some(s => !s.done); f++) { booms.forEach(s => { if (!s.done) API.stepBoomerang(wd2, s, 1 / 60); }); }
  check('ダブルスプーンが正面の敵に当たる', t2.hp < hp0);

  // スプーンが戻れば手ぶら解除
  for (let f = 0; f < 400 && p2.spoonOut; f++) { wd2.shots.filter(s => s.boom).forEach(s => API.stepBoomerang(wd2, s, 1 / 60)); wd2.shots = wd2.shots.filter(s => !s.done); }
  check('スプーンが全部戻れば手ぶら解除', p2.spoonOut === false && (p2.spoonsOut || 0) === 0);
}

console.log('\n=== 112) サムライ居合：突撃対象が死んでも次の敵へ追尾しない（空振り）===');
{
  // 生きている対象は追尾して当てる（従来どおり）
  const wd = API.createWorld(W, H); API.world = wd;
  const s1 = API.makeFighters('daifuku', 'p', W, H, 'army')[0]; s1.appear = 1; s1.x = 100; s1.y = 300;
  s1.cstate = 'dash'; s1.dashT = 0; s1.ddx = 1; s1.ddy = 0;
  const live = API.makeFighters('cookie', 'e', W, H, 'army')[0]; live.appear = 1; live.x = 150; live.y = 300; live.hp = live.maxHp = 9999;
  s1._chargeTgt = live; s1.aimX = live.x; s1.aimY = live.y;
  wd.units.push(s1, live);
  const hp0 = live.hp;
  for (let f = 0; f < 60 && s1.cstate === 'dash'; f++) API.chargerStep(wd, s1, live, 1 / 60);
  check('生きている対象には突撃が当たる（従来どおり）', live.hp < hp0, { hp: live.hp });

  // 対象が死んだら、横にいる次の敵へは曲がらない＝当てない（空振り）
  const wd2 = API.createWorld(W, H); API.world = wd2;
  const s2 = API.makeFighters('daifuku', 'p', W, H, 'army')[0]; s2.appear = 1; s2.x = 100; s2.y = 300;
  s2.cstate = 'dash'; s2.dashT = 0; s2.ddx = 1; s2.ddy = 0;   // 右へ抜刀中
  const deadTgt = API.makeFighters('cookie', 'e', W, H, 'army')[0]; deadTgt.appear = 1; deadTgt.x = 250; deadTgt.y = 300; deadTgt.hp = 0; deadTgt._dead = true;   // 狙った対象は既に死亡
  s2._chargeTgt = deadTgt; s2.aimX = 250; s2.aimY = 300;
  const sideFoe = API.makeFighters('cookie', 'e', W, H, 'army')[0]; sideFoe.appear = 1; sideFoe.x = 150; sideFoe.y = 90; sideFoe.hp = sideFoe.maxHp = 9999;   // 真横（上）にいる次の敵
  wd2.units.push(s2, deadTgt, sideFoe);
  const sideHp0 = sideFoe.hp;
  for (let f = 0; f < 120 && s2.cstate === 'dash'; f++) API.chargerStep(wd2, s2, sideFoe, 1 / 60);   // tgtに横の敵を渡しても…
  check('対象死亡後は横の次の敵へ曲がらない（無傷）', sideFoe.hp === sideHp0, { hp: sideFoe.hp });
  check('空振りしてダッシュ終了（walkへ）', s2.cstate === 'walk');
  check('まっすぐ抜けた（横の敵の方へ曲がっていない：y維持）', Math.abs(s2.y - 300) < 20, { y: s2.y });
}

console.log('\n=== 113) PVP：子は自分の画面幅でステージを表示（親が小さくても縮まない） ===');
{
  // 親が小さい端末（例 320×480）でも、子は自分の画面幅(440)いっぱいに拡大表示
  check('親320×480→子(440)は440×660相当の高さ', API.pvpGuestDisplayH(440, 320, 480) === 660, API.pvpGuestDisplayH(440, 320, 480));
  // 親が大きくても、子の表示幅は自分の画面幅(上限440)に収まる
  check('表示幅は440上限（親500×750でも子440基準）', API.pvpGuestDisplayH(500, 500, 750) === 660, API.pvpGuestDisplayH(500, 500, 750));
  // 子自身が狭い端末(360)なら、その幅×親アスペクト比
  check('子が狭い端末(360)なら360基準', API.pvpGuestDisplayH(360, 440, 660) === 540, API.pvpGuestDisplayH(360, 440, 660));
  // アスペクト比は親のまま保たれる（440×660＝1.5）
  check('親のアスペクト比を保つ', Math.abs(API.pvpGuestDisplayH(440, 440, 660) / 440 - 1.5) < 0.01);
}

console.log('\n=== 114) シェア＆招待（プレイヤー増加の導線） ===');
{
  // シェア文言：勝ち/負けで文面が変わる＋トロフィーは正のときだけ付く
  const w = API.shareResultText(true, 1250);
  const l = API.shareResultText(false, 1250);
  check('勝利文言に「勝利」が入る', w.indexOf('勝利') >= 0, w);
  check('敗北文言に「修行中」が入る', l.indexOf('修行中') >= 0, l);
  check('トロフィー1250が文言に入る', w.indexOf('1250') >= 0, w);
  check('トロフィー0/未指定はトロフィー表記なし', API.shareResultText(true, 0).indexOf('🏆') < 0 && API.shareResultText(true).indexOf('🏆') < 0);

  // 招待URL：base + ?room=CODE（既存クエリ/ハッシュは落とす・大文字化）
  check('招待URLは ?room=CODE', API.roomInviteUrl('https://x.example/g/', 'AB2C') === 'https://x.example/g/?room=AB2C');
  check('招待URLは小文字コードを大文字化', API.roomInviteUrl('https://x.example/', 'ab2c') === 'https://x.example/?room=AB2C');
  check('招待URLは既存の?クエリ/#を除去してから付ける', API.roomInviteUrl('https://x.example/?a=1#h', 'WXYZ') === 'https://x.example/?room=WXYZ');

  // あいことば抽出：4文字[A-Z0-9]のみ有効
  check('parseRoomCode 正常（大文字化）', API.parseRoomCode('?room=ab2c') === 'AB2C');
  check('parseRoomCode 他パラメータと混在', API.parseRoomCode('?x=1&room=WXYZ&y=2') === 'WXYZ');
  check('parseRoomCode 4文字でないものは空', API.parseRoomCode('?room=ABC') === '' && API.parseRoomCode('?room=ABCDE') === '');
...
console.log('\n=== 113) β1.5：X2既定オフ＆クマグミ正式実装＋レッスン ===');
{
  // 通常ゲーム（resetState後）はX2がオフ＝X2候補が出ない
  API.resetState();
  const wd = API.createWorld(W, H); API.world = wd;
  API.makeFighters('cannon', 'p', W, H, 'army').forEach(f => { f.appear = 1; wd.units.push(f); });   // cannon 1体
  API.makeFighters('cannon', 'p', W, H, 'army').forEach(f => { f.appear = 1; wd.units.push(f); });
  API.makeFighters('cannon', 'p', W, H, 'army').forEach(f => { f.appear = 1; wd.units.push(f); });   // 3体（本来X2対象）
  check('通常ゲームはX2既定オフ（state.noX2=true）', API.state.noX2 === true);
  check('X2既定オフなので候補が空', API.eligibleX2Specials().length === 0);
  check('eligibleSpecials にもX2が含まれない', !API.eligibleSpecials().some(k => k.indexOf('x2_') === 0));
  // クマグミ：正式実装＝ロスター/ランダム相手デッキ対象、レッスンで解禁できる
  check('kumagumi は summonOnly でない', !API.UNIT_BY_KEY['kumagumi'].summonOnly);
  check('kumagumi はスターターではない（解禁が必要）', !API.STARTER_UNITS.includes('kumagumi'));
  const arcIds = API.GUIDE_STAGES.filter(s => (s.arc || s.unit) === 'kumagumi').map(s => s.id);
  check('クマグミのレッスンが2段階ある', arcIds.length === 2, arcIds);
  const s2 = API.GUIDE_STAGES.find(s => s.id === 'recruit2');
  check('レッスン2段目でクマグミが解禁される（unit指定）', s2 && s2.unit === 'kumagumi');
  const s1 = API.GUIDE_STAGES.find(s => s.id === 'recruit1');
  check('レッスン1はクマグミ強制＋易化（体験）', s1 && s1.forceUnit === 'kumagumi' && s1.easyFoe === true && s1.winRounds === 1);
  check('レッスン1の相手はクッキーのみ', s1 && JSON.stringify(s1.foe) === JSON.stringify(['cookie']));
}
...
console.log('\n=== 116) 広告リワード（広告を見て🍬・受け皿） ===');
{
  ...
  check('未視聴：used0・remaining=CAP・未上限', (function () {
    const s = API.adRewardState({ adReward: { day: '', count: 0 } }, today);
    return s.used === 0 && s.remaining === API.AD_REWARD_DAILY_CAP && s.capped === false;
  })());
  check('別の日のカウントは今日には持ち越さない', (function () {
    const s = API.adRewardState({ adReward: { day: '2026-7-5', count: API.AD_REWARD_DAILY_CAP } }, today);
    return s.used === 0 && s.capped === false;
  })());
  check('上限まで見たら capped=true・remaining0', (function () {
    const s = API.adRewardState({ adReward: { day: today, count: API.AD_REWARD_DAILY_CAP } }, today);
    return s.capped === true && s.remaining === 0;
  })());

  // grantAdReward：コイン付与＋日次カウント＋上限で打ち止め
  const prof = API.myProfileRef;
  prof.coins = 0; prof.adReward = { day: '', count: 0 };
  const g1 = API.grantAdReward();
  check('1本でAD_REWARD_COINS付与', g1 === API.AD_REWARD_COINS && API.myCoins() === API.AD_REWARD_COINS);
  check('カウントが1に', prof.adReward.count === 1);
  // 上限まで回す
  let guard = 0;
  while (API.adRewardState(prof, API.myProfileRef.adReward.day).capped === false && guard++ < 50) API.grantAdReward();
  check('上限到達後はコインが増えない（0を返す）', API.grantAdReward() === 0 && prof.adReward.count === API.AD_REWARD_DAILY_CAP);
...
console.log('\n=== 121) シェルマカロンの殻ぱかぱか（移動中） ===');
{
  API.resetState(); API.setupCanvas();
  const W = API.CW_get() || 440, H = API.CH_get() || 760;
  if (API.SPRITES['macaron_blue'] && API.SPRITES['macaron_open_blue']) {
    ...
    // 殻を脱いだ後（normal）＋移動中はぱかぱか（閉↔開）
    u.shellPhase = 'normal'; u.mv = true;
    w.t = 0;   // floor(0)%2=0 → 閉
    check('normal移動中 位相0で 閉(macaron_blue)', API.spriteFor(u) === API.SPRITES['macaron_blue']);
    w.t = 1 / API.MAC_PAKA_FPS();   // floor(1)%2=1 → 開
    check('normal移動中で 開(macaron_open_blue)に切替', API.spriteFor(u) === API.SPRITES['macaron_open_blue']);
    // 止まっているときは閉じ
    u.mv = false; w.t = 1 / API.MAC_PAKA_FPS();
    check('停止中は 閉(macaron_blue)', API.spriteFor(u) === API.SPRITES['macaron_blue']);
    // 殻スピン/スタン中は移動中でも閉じた殻（ぱかぱかしない）
    u.shellPhase = 'spin'; u.mv = true; w.t = 1 / API.MAC_PAKA_FPS();
    check('殻スピン中はぱかぱかせず macaron_spin', API.spriteFor(u) === API.SPRITES['macaron_spin_blue']);
    u.shellPhase = 'stun';
    check('スタン中も macaron_spin', API.spriteFor(u) === API.SPRITES['macaron_spin_blue']);
    // 敵（赤）版
    const e = API.makeFighters('macaron', 'e', W, H, 'army')[0]; e.appear = 1; e.x = 0; e.y = H * 0.3;
...
console.log('\n=== 123) ショートケーキスネイルの歩行＋設置切替 ===');
{
  API.resetState(); API.setupCanvas();
  const W = API.CW_get() || 440, H = API.CH_get() || 760;
  if (API.SPRITES['shortcake_blue'] && API.SPRITES['shortcake2_blue']) {
    const w = API.createWorld(W, H); API.world = w;
    const u = API.makeFighters('shortcake', 'p', W, H, 'army')[0]; u.appear = 1; u.x = 0; u.y = H * 0.6;
    // イチゴを背負う（未設置）＝carry。戦闘中は歩行2フレーム
    u.myTurret = null; w.phase = 'battle'; w.t = 0;
    check('carry 戦闘中 位相0で walk1(shortcake_blue)', API.spriteFor(u) === API.SPRITES['shortcake_blue']);
    w.t = 1 / API.SHORTCAKE_WALK_FPS();
    check('carry 戦闘中で walk2(shortcake2_blue)に切替', API.spriteFor(u) === API.SPRITES['shortcake2_blue']);
    // タレット設置中＝イチゴを下ろした立ち絵(deploy)で歩行
    u.myTurret = { hp: 70 }; w.t = 0;
    check('deploy 戦闘中 位相0で deploy1(shortcake_deploy_blue)', API.spriteFor(u) === API.SPRITES['shortcake_deploy_blue']);
    w.t = 1 / API.SHORTCAKE_WALK_FPS();
    check('deploy 戦闘中で deploy2(shortcake_deploy2_blue)に切替', API.spriteFor(u) === API.SPRITES['shortcake_deploy2_blue']);
    // 待機中は frame1（背負い）
    u.myTurret = null; w.phase = 'muster'; w.t = 1 / API.SHORTCAKE_WALK_FPS();
    check('待機中は frame1(shortcake_blue)', API.spriteFor(u) === API.SPRITES['shortcake_blue']);
    // 敵（赤・いちご）
    const e = API.makeFighters('shortcake', 'e', W, H, 'army')[0]; e.appear = 1; e.x = 0; e.y = H * 0.3;
    e.myTurret = null; w.phase = 'battle'; w.t = 0;
    check('敵: 戦闘中で赤 walk1(shortcake_red)', API.spriteFor(e) === API.SPRITES['shortcake_red']);
    // タレット
    check('タレットは strawturret_blue に解決', API.spriteFor(API.makeFighters('strawturret','p',W,H,'army')[0]) === API.SPRITES['strawturret_blue']);
  }
  check('10フレーム（carry/deploy×walk1/2＋turret ×青赤）が SPRITE_DATA に存在',
    ['shortcake_blue','shortcake_red','shortcake2_blue','shortcake2_red',
     'shortcake_deploy_blue','shortcake_deploy_red','shortcake_deploy2_blue','shortcake_deploy2_red',
     'strawturret_blue','strawturret_red'].every(k => !!API.SPRITE_DATA[k]));
}
...
console.log('\n=== 124) ゴーストはワープ後の無敵中は動かない ===');
{
  ...
  const W = API.CW_get() || 440, H = API.CH_get() || 760;
  const w = API.createWorld(W, H); w.phase = 'battle'; w.intro = 0; API.world = w;
  const g = API.makeFighters('ghost', 'p', W, H, 'army')[0]; g.appear = 1;
  g.warped = true; g.warpT = 99; g.x = W * 0.5; g.y = H * 0.2;   // ワープ済み扱い
  g.invuln = 0.6;   // ワープ直後の無敵
  w.units.push(g);
  // 動きたくなるよう遠くに敵を置く
  const e = API.makeFighters('cookie', 'e', W, H, 'army')[0]; e.appear = 1; e.x = W * 0.5; e.y = H * 0.8; w.units.push(e);
  const x0 = g.x, y0 = g.y;
  for (let i = 0; i < 6; i++) API.stepWorld(w, 1 / 60);   // 0.1秒ぶん（まだinvuln>0）
  check('無敵中(invuln>0)はゴーストが動かない', g.invuln > 0 && Math.hypot(g.x - x0, g.y - y0) < 1.5);
  // 無敵を切らして進めると動き出す
  g.invuln = 0; g.x = x0; g.y = y0;
  for (let i = 0; i < 20; i++) API.stepWorld(w, 1 / 60);
...
console.log('\n=== 125) イチゴタレットは当たり判定なし（素通し・盾にならない） ===');
{
  ...
  // マシュマロエンジェルも当たり判定なし（noCollide）
  const w2 = API.createWorld(W, H); w2.phase = 'battle'; w2.intro = 0; API.world = w2;
  const angel = API.makeFighters('mangel', 'p', W, H, 'army')[0]; angel.appear = 1; angel.x = W * 0.5; angel.y = H * 0.5; w2.units.push(angel);
  const mfoe = API.makeFighters('cookie', 'e', W, H, 'army')[0]; mfoe.appear = 1; mfoe.x = W * 0.5; mfoe.y = H * 0.5; w2.units.push(mfoe);
  const mx0 = mfoe.x, my0 = mfoe.y;
  check('マシュマロエンジェルは noCollide', angel.noCollide === true);
  for (let i = 0; i < 4; i++) API.resolveOverlaps(w2);
  check('マシュマロエンジェルは敵を押し出さない', Math.hypot(mfoe.x - mx0, mfoe.y - my0) < 0.5);
}

console.log('\n=== 126) ショートケーキ固有強化「練乳ベリー」：タレット強化＋立ち絵切替 ===');
{
  API.resetState(); API.setupCanvas();
  const W = API.CW_get() || 440, H = API.CH_get() || 760;
  check('buff_shortcake が SPECIALS に存在', !!API.SPECIALS['buff_shortcake'] && API.SPECIALS['buff_shortcake'].shortcakeBuff === true);
  check('強化名は「練乳ベリー」', API.SPECIALS['buff_shortcake'].name === '練乳ベリー');
  check('練乳ベリーの4スプライトが SPRITE_DATA に存在',
    ['strawturret_buff_blue','strawturret_buff_red','shortcake_buff_blue','shortcake_buff_red'].every(k => !!API.SPRITE_DATA[k]));
  // 盤面にショートケーキが居れば提示候補に入る
  const w0 = API.createWorld(W, H); API.world = w0;
  API.makeFighters('shortcake', 'p', W, H, 'army').forEach(f => { f.appear = 1; w0.units.push(f); });
  check('ショートケーキが居れば buff_shortcake が提示候補に', API.eligibleSpecials().includes('buff_shortcake'));
  // 強化フラグ適用
  const w = API.createWorld(W, H); w.phase = 'battle'; w.intro = 0; API.world = w;
  const sc = API.makeFighters('shortcake', 'p', W, H, 'army')[0]; sc.appear = 1; sc.x = W * 0.5; sc.y = H * 0.6; w.units.push(sc);
  API.applyShortcakeBuff(w, 'p');
  check('applyShortcakeBuffで強化フラグ', sc.shortcakeBuff === true);
  // 強化ショートケーキが置くタレットは atk↑・cd↓
  const t = API.deployTurret(w, sc);
  check('強化タレットは攻撃力UP', t.atk === API.SHORTCAKE_TURRET_ATK() && t.atk > API.UNIT_BY_KEY['strawturret'].atk);
  check('強化タレットは攻撃間隔ダウン（速射）', t.cd === API.SHORTCAKE_TURRET_CD() && t.cd < API.UNIT_BY_KEY['strawturret'].cd);
  // 未強化ショートケーキのタレットは通常値
  const sc2 = API.makeFighters('shortcake', 'p', W, H, 'army')[0]; sc2.appear = 1; sc2.x = W * 0.3; sc2.y = H * 0.6; w.units.push(sc2);
  const t2 = API.deployTurret(w, sc2);
  check('未強化タレットは通常のatk/cd', t2.atk === API.UNIT_BY_KEY['strawturret'].atk && t2.cd === API.UNIT_BY_KEY['strawturret'].cd);
  // 立ち絵切替：強化タレットは t.shortcakeBuff → strawturret_buff_、強化本体(未設置)は shortcake_buff_
  if (API.SPRITES['strawturret_buff_blue'] && API.SPRITES['shortcake_buff_blue']) {
    check('強化タレットに shortcakeBuff フラグが立つ', t.shortcakeBuff === true);
    check('強化タレットの立ち絵は strawturret_buff', API.spriteFor(t) === API.SPRITES['strawturret_buff_blue']);
    check('未強化タレットは通常立ち絵', API.spriteFor(t2) === API.SPRITES['strawturret_blue']);
    // 強化本体・未設置（myTurret無し）＝練乳ベリー背負い立ち絵
    const sc3 = API.makeFighters('shortcake', 'p', W, H, 'army')[0]; sc3.appear = 1; sc3.x = 0; sc3.y = H * 0.6; sc3.shortcakeBuff = true; sc3.myTurret = null; w.phase = 'muster';
    check('強化本体(未設置)は shortcake_buff 立ち絵', API.spriteFor(sc3) === API.SPRITES['shortcake_buff_blue']);
    // 設置中はベリーを下ろしているので通常deploy（練乳は下ろしたタレット側）
    sc3.myTurret = { hp: 70 };
    check('強化本体(設置中)は通常deploy立ち絵', API.spriteFor(sc3) === API.SPRITES['shortcake_deploy_blue']);
  }
  check('shortcakeBuff が SNAP_UNIT_FIELDS 経由でスナップショットに乗る', (function(){
    const ws = API.createWorld(W, H); ws.phase = 'battle'; API.world = ws;
    const u = API.makeFighters('shortcake', 'p', W, H, 'army')[0]; u.appear = 1; u.shortcakeBuff = true; ws.units.push(u);
    const snap = API.serializeWorld(ws); const su = snap.units.find(x => x.key === 'shortcake');
    return !!su && su.shortcakeBuff === true;
  })());
}
...
console.log('\n=== 127) カヌレモーラー：潜行→浮上の潜行強襲 ===');
{
  API.resetState(); API.setupCanvas();
  const W = API.CW_get() || 440, H = API.CH_get() || 760;
  check('canule は 1枚で2体（count:2）', API.UNIT_BY_KEY['canule'].count === 2);
  check('canule は正式登場（test:trueでない・β1.6）', !API.UNIT_BY_KEY['canule'].test);
  check('canule の4スプライトが存在', ['canule_blue','canule_red','canule_buff_blue','canule_buff_red'].every(k => !!API.SPRITE_DATA[k]));
  const w = API.createWorld(W, H); w.phase = 'battle'; w.intro = 0; API.world = w;
  const m = API.makeFighters('canule', 'p', W, H, 'army')[0]; m.appear = 1; m.x = W * 0.5; m.y = H * 0.75; w.units.push(m);
  const e = API.makeFighters('cookie', 'e', W, H, 'army')[0]; e.appear = 1; e.x = W * 0.5; e.y = H * 0.18; e.hp = e.maxHp = e.baseMaxHp = 1e9; e.speed = 0; e.baseSpeed = 0; w.units.push(e);
  // 開幕は地上(surface)＝無敵ではない（狙われる溜め）
  API.stepWorld(w, 1 / 60);
  check('開幕は地上(surface)＝無敵でない', m.moleState === 'surface' && !(m.invuln > 0));
  // 約2秒後に土へ潜る(dig)＝無敵＆当たり判定なし
  for (let i = 0; i < 130; i++) { if (m.moleState === 'dig') break; API.stepWorld(w, 1 / 60); }
  check('約2秒で潜行(dig)に入る', m.moleState === 'dig');
  check('潜行中は無敵(invuln>0)', m.invuln > 0);
  check('潜行中は当たり判定なし(noCollide)', m.noCollide === true);
  // 潜行中は敵(上)へ近づく
  const y0 = m.y;
  for (let i = 0; i < 60 && m.moleState === 'dig'; i++) API.stepWorld(w, 1 / 60);
  check('潜行中は敵へ近づく（y減少）', m.y < y0 - 5);
  // 十分近づけば浮上(attack)＝無敵解除・通常戦闘へ
  for (let i = 0; i < 600 && m.moleState !== 'attack'; i++) API.stepWorld(w, 1 / 60);
  check('接近で浮上(attack)＝無敵/すり抜け解除', m.moleState === 'attack' && !(m.invuln > 0) && m.noCollide === false);
...
console.log('\n=== 127d) カヌレ同士は地中(潜行中)の相手も察知できる ===');
{
  API.resetState(); API.setupCanvas();
  const W = API.CW_get() || 440, H = API.CH_get() || 760;
  const w = API.createWorld(W, H); w.phase = 'battle'; w.intro = 0; API.world = w;
  // 敵カヌレを地中(潜行=invuln)に置く
  const emole = API.makeFighters('canule', 'e', W, H, 'army')[0]; emole.appear = 1; emole.x = W * 0.5; emole.y = H * 0.4; emole.moleState = 'dig'; emole.invuln = 0.5; emole.noCollide = true; w.units.push(emole);
  // 味方カヌレ（探す側）
  const pmole = API.makeFighters('canule', 'p', W, H, 'army')[0]; pmole.appear = 1; pmole.x = W * 0.5; pmole.y = H * 0.7; w.units.push(pmole);
  check('カヌレは地中の敵カヌレを察知(nearestEnemyで拾える)', API.nearestEnemy(pmole, w) === emole);
  // 一方、通常ユニット(チョコ)は地中のカヌレを察知できない（従来どおり無敵はスルー）
  const choco = API.makeFighters('choco', 'p', W, H, 'army')[0]; choco.appear = 1; choco.x = W * 0.5; choco.y = H * 0.7;
  check('通常ユニットは地中のカヌレを察知しない', API.nearestEnemy(choco, w) === null);
  // 地上(surface/attack=invulnなし)のカヌレは誰でも通常どおり狙える
  emole.moleState = 'attack'; emole.invuln = 0;
  check('浮上したカヌレは通常ユニットも狙える', API.nearestEnemy(choco, w) === emole);
}

console.log('\n=== 128) 呪文（スペル）：下準備フェーズ（テスト対戦限定・プレイヤーのみ） ===');
{
  // --- データと初期化 ---
  check('SPELL_ORDER に thunder / reroll がある', API.SPELL_ORDER.includes('thunder') && API.SPELL_ORDER.includes('reroll'));
  check('雷おこしは1ピック消費（keepPickなし）', !API.SPELLS.thunder.keepPick);
  check('カードリロールはピック消費しない（keepPick）', API.SPELLS.reroll.keepPick === true);
  API.resetState();
  check('resetStateで呪文は既定オフ', API.state.spellsOn === false);
  check('spellUsesが各呪文の既定回数で初期化', API.state.spellUses.thunder === API.SPELLS.thunder.uses && API.state.spellUses.reroll === API.SPELLS.reroll.uses);
  // 呪文オフのときは使えない（通常CPU/PVPに影響しない）
  check('呪文オフのときcanCastSpellはfalse', API.canCastSpell('thunder') === false);

  // --- ⚡雷おこし：発動時は「開戦時の落雷」を予約（即ダメージではない） ---
  API.setupCanvas();
  const W = API.CW_get() || 440, H = API.CH_get() || 760;
  let w = API.createWorld(W, H); w.phase = 'muster'; API.world = w;
  API.state.spellsOn = true; API.state.thunderPending = 0;
  // 高HPのチョコを敵に8体（60ダメージでは死なない＝被弾を数えられる）
  const foes = [];
  for (let i = 0; i < 4; i++) API.makeFighters('choco', 'e', W, H, 'army').forEach(f => { f.appear = 1; f.x = 40 + foes.length * 20; f.y = 60; w.units.push(f); foes.push(f); });
  // 無敵の敵・狙えない敵（タレット相当）は必ず除外される
  const invu = API.makeFighters('choco', 'e', W, H, 'army')[0]; invu.appear = 1; invu.invuln = 1; invu.x = 300; invu.y = 60; w.units.push(invu);
  const untg = API.makeFighters('choco', 'e', W, H, 'army')[0]; untg.appear = 1; untg.untargetable = true; untg.x = 320; untg.y = 60; w.units.push(untg);
  API.castThunder();
  check('雷おこしは発動時に落雷を1回予約', API.state.thunderPending === 1);
  check('発動しただけでは即ダメージは無い（開戦時に落ちる）', foes.every(f => f.hp === f.maxHp));
  // 開戦直前の一斉落雷（fireThunderStrikes）
  const hit = API.fireThunderStrikes(w, API.state.thunderPending);
  check('開戦時の落雷は最大5体（敵8体→ちょうど5体）', hit === API.SPELL_THUNDER_TARGETS);
  const damaged = foes.filter(f => f.hp < f.maxHp).length;
  check('被弾したのはちょうど5体', damaged === 5);
  check('無敵の敵は落雷を受けない', invu.hp === invu.maxHp);
  check('狙えない敵（タレット等）は落雷を受けない', untg.hp === untg.maxHp);
  check('落雷で稲妻エフェクト(bolts)が積まれる', Array.isArray(w.bolts) && w.bolts.length === 5);

  // 敵が5体未満なら全員に落ちる
  let w2 = API.createWorld(W, H); w2.phase = 'muster'; API.world = w2;
  const three = [];
  API.makeFighters('choco', 'e', W, H, 'army').forEach(f => { f.appear = 1; w2.units.push(f); three.push(f); });   // 2体
  API.makeFighters('daifuku', 'e', W, H, 'army').forEach(f => { f.appear = 1; w2.units.push(f); three.push(f); });  // 1体
  check('敵が5体未満なら全員に落雷', API.fireThunderStrikes(w2, 1) === three.length);

  // --- castSpell 統合：消費タイプはピックを1つ進め回数を1消費＋落雷を予約 ---
  let w3 = API.createWorld(W, H); w3.phase = 'muster'; API.world = w3;
  API.makeFighters('choco', 'e', W, H, 'army').forEach(f => { f.appear = 1; w3.units.push(f); });
  API.state.spellsOn = true; API.state.spellUses = { thunder: 2, reroll: 3 }; API.state.thunderPending = 0;
  API.state.loadout = ['cookie', 'shoe', 'choco', 'bomb']; API.state.offer3 = ['cookie', 'shoe', 'choco'];
  API.state.pickStep = 0; API.state.pickTotal = 3; API.state.playerExtra = 0;
  API.state.foeRoundPicks = []; API.state.foeRevealIdx = 0;
  check('消費タイプはcanCastSpell true（提示中・残りあり）', API.canCastSpell('thunder') === true);
  API.castSpell('thunder');
  check('雷おこしで回数が2→1に減る', API.state.spellUses.thunder === 1);
  check('雷おこしで1ピック消費（pickStep 0→1）', API.state.pickStep === 1);
  check('雷おこしで落雷が予約される（thunderPending 1）', API.state.thunderPending === 1);

  // --- 1ラウンド1回：雷おこしは使ったら同ラウンド中は再使用不可（残り回数があっても） ---
  check('雷おこしは同ラウンド使用済み', API.spellUsedThisRound('thunder') === true);
  check('残り回数があっても同ラウンドは再使用不可', API.state.spellUses.thunder === 1 && API.canCastSpell('thunder') === false);
  API.state.spellRoundUsed = {};   // 次ラウンド相当（beginDraftでリセットされる）
  check('ラウンドが変われば再び使える', API.canCastSpell('thunder') === true);

  // --- 干渉系（カードリロール）は oncePerRound なし＝同ラウンド中も繰り返し使える ---
  let wr = API.createWorld(W, H); wr.phase = 'muster'; API.world = wr;
  API.state.spellUses = { thunder: 1, reroll: 3 }; API.state.spellRoundUsed = {};
  API.state.loadout = ['cookie', 'shoe', 'choco', 'bomb']; API.state.offer3 = ['cookie', 'shoe', 'choco'];
  API.state.pickStep = 0; API.state.pickTotal = 3;
  API.castSpell('reroll');
  check('カードリロールは同ラウンドでも続けて使える', API.canCastSpell('reroll') === true);

  // --- castSpell 統合：keepPickタイプは提示を引き直しピックは消費しない ---
  let w4 = API.createWorld(W, H); w4.phase = 'muster'; API.world = w4;
  API.state.spellsOn = true; API.state.spellUses = { thunder: 2, reroll: 3 };
  API.state.loadout = ['cookie', 'shoe', 'choco', 'bomb']; API.state.offer3 = ['cookie', 'shoe', 'choco'];
  API.state.pickStep = 1; API.state.pickTotal = 3;
  API.castSpell('reroll');
  check('カードリロールで回数が3→2に減る', API.state.spellUses.reroll === 2);
  check('カードリロールはピックを消費しない（pickStep維持）', API.state.pickStep === 1);
  check('カードリロール後も提示は3枚', API.state.offer3.length === 3);
  check('引き直した提示はデッキ内のカード', API.state.offer3.every(k => API.state.loadout.includes(k) || API.isSpecial(k)));

  // --- 使い切り：残り0では使えない ---
  API.state.spellUses.thunder = 0;
  check('残り0の呪文はcanCastSpell false', API.canCastSpell('thunder') === false);
  // 戦闘中（phase!=='muster'）は使えない
  w4.phase = 'battle';
  check('戦闘中は呪文を使えない', API.canCastSpell('reroll') === false);
}

console.log('\n=== 129) 新キャラ：エクレアドッグ＆ワッフルランサー（開発テスト中） ===');
{
  const dog = API.UNIT_BY_KEY['eclairdog'];
  const lancer = API.UNIT_BY_KEY['wafflelancer'];
  check('eclairdog / wafflelancer が UNITS にある', !!dog && !!lancer);
  check('両者とも test:true（通常プレイ非表示）', dog.test === true && lancer.test === true);
  const normalKeys = API.UNITS.filter(u => !u.summonOnly && !u.test).map(u => u.key);
  check('通常ロスターに含まれない', normalKeys.indexOf('eclairdog') < 0 && normalKeys.indexOf('wafflelancer') < 0);
  check('X2カードが生成されない', !API.SPECIALS.x2_eclairdog && !API.SPECIALS.x2_wafflelancer);

  check('eclairdog は複数(count>=3)で高速(speed>=140)', dog.count >= 3 && dog.speed >= 140);
  check('eclairdog tier はゼリー(0.5)の1つ前あたり(0<tier<0.5)', dog.tier > 0 && dog.tier < 0.5, dog.tier);
  check('eclairdog 走り2フレームが SPRITE_DATA にある（青/赤）',
    ['eclairdog_blue','eclairdog_run_blue','eclairdog_red','eclairdog_run_red'].every(k => !!API.SPRITE_DATA[k]));
  {
    const df = API.makeFighters('eclairdog', 'p', W, H, 'army')[0];
    const sp = API.spriteFor(df);
    check('eclairdog の spriteFor が走りアニメのフレームを返す',
      sp === API.SPRITES['eclairdog_blue'] || sp === API.SPRITES['eclairdog_run_blue']);
  }

  check('wafflelancer は charger + pierce=1（突撃は単体に大ダメージ）', lancer.charger === true && lancer.pierce === 1);
  check('wafflelancer tier はクッキー(-0.5)の後ろあたり(-0.5<tier<0)', lancer.tier > -0.5 && lancer.tier < 0, lancer.tier);
  check('wafflelancer 本体(青/赤)＋槍スプライトが SPRITE_DATA にある',
    ['wafflelancer_body_blue','wafflelancer_body_red','wafflelancer_lance'].every(k => !!API.SPRITE_DATA[k]));
  {
    const lf = API.makeFighters('wafflelancer', 'p', W, H, 'army')[0];
    check('fighter に pierce がコピーされる', lf.pierce === 1);
    check('wafflelancer の spriteFor が槍なし本体を返す', API.spriteFor(lf) === API.SPRITES['wafflelancer_body_blue']);
  }

  {
    const wl = API.createWorld(W, H); API.world = wl;
    const lf = API.makeFighters('wafflelancer', 'p', W, H, 'army')[0];
    lf.x = W / 2; lf.y = H - 30; lf.appear = 1; wl.units.push(lf);
    const foes = [];
    for (let i = 0; i < 5; i++) {
      const e = API.makeFighters('cookie', 'e', W, H, 'army')[0];
      e.x = W / 2; e.y = H - 30 - 34 * (i + 1); e.appear = 1; e.speed = 0; e.baseSpeed = 0; e.hp = e.maxHp = 500;
      wl.units.push(e); foes.push(e);
    }
    wl.phase = 'battle'; wl.intro = 0; wl.t = 0; wl.done = false;
    let prev = foes.map(f => f.hp), maxHit = 0;
    let frames = 0; while (frames < 60 * 4 && !wl.done) { API.stepWorld(wl, 1 / 60); foes.forEach((f, i) => { if (prev[i] - f.hp > maxHit) maxHit = prev[i] - f.hp; prev[i] = f.hp; }); frames++; }
    check('突撃は単体に大ダメージ（1回で150前後）', maxHit >= 150, { maxHit });
    check('ランサーは詰まらず前進して敵陣に食い込む（y が前進）', lf.y < H - 30, { y: Math.round(lf.y) });
  }

  // 強化後の数値（弱すぎ調整）
  check('eclairdog: hp30 / cd0.3 / firstHit20 / spriteScale1.6', dog.hp === 30 && dog.cd === 0.3 && dog.firstHit === 20 && dog.spriteScale === 1.6,
    { hp: dog.hp, cd: dog.cd, firstHit: dog.firstHit, scale: dog.spriteScale });
  check('wafflelancer: atk30 / hp250 / dashDamage150 / pierce1（ナーフ撤回＋単体突撃）', lancer.atk === 30 && lancer.hp === 250 && lancer.dashDamage === 150 && lancer.pierce === 1,
    { atk: lancer.atk, hp: lancer.hp, dash: lancer.dashDamage, pierce: lancer.pierce });
  // 大福サムライ：通常攻撃が前方範囲（ランサーとの差別化）
  check('大福は cleaveNormal=true（通常攻撃も前方範囲）', API.UNIT_BY_KEY['daifuku'].cleaveNormal === true);
  check('ワッフルランサーは cleaveNormal でない（通常は単体）', !API.UNIT_BY_KEY['wafflelancer'].cleaveNormal);
  {
    const w = API.createWorld(W, H); API.world = w;
    const sam = API.makeFighters('daifuku', 'p', W, H, 'army')[0]; sam.x = W / 2; sam.y = H / 2; sam.appear = 1; sam.cstate = 'walk'; sam.cool = 0; w.units.push(sam);
    const es = []; for (let i = 0; i < 3; i++) { const e = API.makeFighters('cookie', 'e', W, H, 'army')[0]; e.x = W / 2 - 16 + i * 16; e.y = H / 2 - 20; e.appear = 1; e.speed = 0; e.baseSpeed = 0; e.hp = e.maxHp = 500; w.units.push(e); es.push(e); }
    w.phase = 'battle'; w.intro = 0; w.done = false;
    const s0 = es.map(e => e.hp);
    for (let i = 0; i < 60 * 3 && !w.done; i++) API.stepWorld(w, 1 / 60);
    const hit = es.filter((e, i) => e.hp < s0[i]).length;
    check('サムライの前方範囲攻撃が複数の敵に当たる（2体以上）', hit >= 2, { hit });
  }

  // firstHit：最初の噛みつきだけ20、2撃目以降は通常10
  {
    const wl = API.createWorld(W, H); API.world = wl;
    const dg = API.makeFighters('eclairdog', 'p', W, H, 'army')[0];
    dg.x = W / 2; dg.y = H / 2; dg.appear = 1; dg.cool = 0; wl.units.push(dg);
    check('fighter に firstHit がコピー＆初期 bitten=false', dg.firstHit === 20 && dg.bitten === false);
    const e = API.makeFighters('cookie', 'e', W, H, 'army')[0];
    e.x = W / 2; e.y = H / 2 - 12; e.appear = 1; e.speed = 0; e.baseSpeed = 0; e.hp = e.maxHp = 1000; wl.units.push(e);
    wl.phase = 'battle'; wl.intro = 0; wl.done = false;
    const drops = []; let prev = e.hp;
    for (let i = 0; i < 180 && drops.length < 2; i++) { API.stepWorld(wl, 1 / 60); if (e.hp < prev) { drops.push(prev - e.hp); prev = e.hp; } }
    check('最初の噛みつきは20ダメージ（firstHit）', drops[0] === 20, drops);
    check('2撃目以降は通常10ダメージ', drops[1] === 10, drops);
  }
}

console.log('\n=== 130) 初の空中ユニット：琥珀バチ（開発テスト中） ===');
{
  const bee = API.UNIT_BY_KEY['amberbee'];
  check('amberbee が UNITS にある・test:true', !!bee && bee.test === true);
  check('amberbee は fly + noCollide（飛行）', bee.fly === true && bee.noCollide === true);
  check('amberbee は攻撃する飛行スウォーム（noAttackでない・複数・atk>0）', !bee.noAttack && bee.count >= 2 && bee.atk > 0);
  {
    const bf = API.makeFighters('amberbee', 'p', W, H, 'army')[0];
    check('fighter に fly/noCollide が乗る', bf.fly === true && bf.noCollide === true);
  }
  // 近接は飛行を狙えない／遠距離は狙える（fly の肝＝ガーゴイル特性）
  {
    const w = API.createWorld(W, H); API.world = w;
    const beeE = API.makeFighters('amberbee', 'e', W, H, 'army')[0]; beeE.x = W / 2; beeE.y = H / 2; beeE.appear = 1; w.units.push(beeE);
    const ck = API.makeFighters('cookie', 'p', W, H, 'army')[0]; ck.x = W / 2; ck.y = H / 2 + 12; ck.appear = 1; w.units.push(ck);
    check('近接(cookie)は飛行(amberbee)を標的にできない', API.nearestEnemy(ck, w) !== beeE);
    const sh = API.makeFighters('shoe', 'p', W, H, 'army')[0]; sh.x = W / 2; sh.y = H / 2 + 12; sh.appear = 1; w.units.push(sh);
    check('遠距離(shoe)は飛行(amberbee)を標的にできる', API.nearestEnemy(sh, w) === beeE);
  }
  // 相性（実測）：近接軍(cookie)には一方的に強い／遠距離(shoe)・AoE(bomb)には弱い
  {
    function battle(kP, kE, picks) {
      const w = API.createWorld(W, H); API.world = w;
      for (let i = 0; i < picks; i++) { API.makeFighters(kP, 'p', W, H, 'army').forEach(f => { f.appear = 1; w.units.push(f); }); API.makeFighters(kE, 'e', W, H, 'army').forEach(f => { f.appear = 1; w.units.push(f); }); }
      API.arrangeFormation(w, 'p', true); API.arrangeFormation(w, 'e', true); w.phase = 'battle'; w.intro = 0; w.done = false;
      let fr = 0; while (!w.done && fr < 60 * 45) { API.stepWorld(w, 1 / 60); fr++; } return w.done ? w.result : 'stuck';
    }
    function rate(kE, tr) { let win = 0; for (let g = 0; g < tr; g++) { if (battle('amberbee', kE, 4) === 'win') win++; } return win / tr; }
    const vsCookie = rate('cookie', 16), vsShoe = rate('shoe', 16), vsBomb = rate('bomb', 16), vsChoco = rate('choco', 16);
    console.log('  [琥珀バチ相性] vs cookie(近接):' + Math.round(vsCookie * 100) + '%  vs choco(近接タンク):' + Math.round(vsChoco * 100) + '%  vs shoe(遠距離):' + Math.round(vsShoe * 100) + '%  vs bomb(AoE):' + Math.round(vsBomb * 100) + '%');
    check('近接軍(cookie)に強い（勝率>0.7）', vsCookie > 0.7, vsCookie.toFixed(2));
    check('遠距離(shoe)に弱い（勝率<0.45）', vsShoe < 0.45, vsShoe.toFixed(2));
  }
}

console.log('\n=== 131) 新ユニット：バウムクーヘンゴーレム／ラムネ大砲＋隊列 ===');
{
  const baum = API.UNIT_BY_KEY['baumgolem'];
  const ram = API.UNIT_BY_KEY['ramunecannon'];
  const bee = API.UNIT_BY_KEY['amberbee'];
  const cookie = API.UNIT_BY_KEY['cookie'];
  const choco = API.UNIT_BY_KEY['choco'];
  check('baumgolem / ramunecannon が UNITS にある・test:true', !!baum && !!ram && baum.test === true && ram.test === true);
  // バウムクーヘンゴーレム＝超硬タンク・クッキーの前
  check('baumgolem は超硬タンク（HP最大級・heavy・kbResist）', baum.hp >= 500 && baum.heavy === true && baum.kbResist === true);
  check('baumgolem はクッキーの前（tier < cookie）', baum.tier < cookie.tier, { baum: baum.tier, cookie: cookie.tier });
  // ラムネ大砲＝建設物（不動）・チョコの後ろ・遠距離
  check('ramunecannon は不動の遠距離建設物（speed0・ranged・長射程）', ram.speed === 0 && ram.ranged === true && ram.range >= 180);
  check('ramunecannon の火力（atk60 / cd1.0＝DPS60）', ram.atk === 60 && ram.cd === 1.0);
  check('ramunecannon はチョコの後ろ（tier > choco）', ram.tier > choco.tier, { ram: ram.tier, choco: choco.tier });
  // 琥珀バチはラムネの後ろ
  check('琥珀バチはラムネ大砲の後ろ（tier > ramunecannon）', bee.tier > ram.tier, { bee: bee.tier, ram: ram.tier });
  // 隊列の実配置：baum が cookie より前（y が敵側=小さい）… arrangeFormation で確認
  {
    const w = API.createWorld(W, H); API.world = w;
    API.makeFighters('baumgolem', 'p', W, H, 'army').forEach(f => { f.appear = 1; w.units.push(f); });
    API.makeFighters('cookie', 'p', W, H, 'army').forEach(f => { f.appear = 1; w.units.push(f); });
    API.makeFighters('ramunecannon', 'p', W, H, 'army').forEach(f => { f.appear = 1; w.units.push(f); });
    API.arrangeFormation(w, 'p', true);
    const by = w.units.find(u => u.key === 'baumgolem').y;
    const cy = w.units.filter(u => u.key === 'cookie').reduce((a, u) => a + u.y, 0) / 5;
    const ry = w.units.find(u => u.key === 'ramunecannon').y;
    check('隊列：バウム(前)はクッキーより敵側(y小)', by < cy, { by: Math.round(by), cy: Math.round(cy) });
    check('隊列：ラムネ大砲(後)はクッキーより自陣側(y大)', ry > cy, { ry: Math.round(ry), cy: Math.round(cy) });
  }
  // ラムネ大砲は不動で遠距離から撃つ（近接軍を溶かす）
  {
    const w = API.createWorld(W, H); API.world = w;
    const r = API.makeFighters('ramunecannon', 'p', W, H, 'army')[0]; r.x = W / 2; r.y = H * 0.7; r.appear = 1; w.units.push(r);
    const e = API.makeFighters('cookie', 'e', W, H, 'army')[0]; e.x = W / 2; e.y = H * 0.7 - 110; e.appear = 1; e.speed = 0; e.baseSpeed = 0; e.hp = e.maxHp = 500; w.units.push(e);
    w.phase = 'battle'; w.intro = 0; w.done = false;
    const rx = r.x, ry = r.y, eh = e.hp;
    for (let i = 0; i < 60 * 5 && !w.done; i++) API.stepWorld(w, 1 / 60);
    const drift = Math.round(Math.hypot(r.x - rx, r.y - ry) * 10) / 10;
