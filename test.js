...
;globalThis.__API = {
  ...
  spriteFor, get SPRITES(){ return SPRITES; }, slimeHopPhase, slimeHopLift, SLIME_HOP_FPS:()=>SLIME_HOP_FPS, BOMB_WALK_FPS:()=>BOMB_WALK_FPS, MAC_PAKA_FPS:()=>MAC_PAKA_FPS, SODA_WALK_FPS:()=>SODA_WALK_FPS, SHORTCAKE_WALK_FPS:()=>SHORTCAKE_WALK_FPS,
...
console.log('\n=== 95) アニメ付きキャラ（マシュマロエンジェル）：フレーム切替＋テスト除外 ===');
{
  ...
  // テスト用キャラは2体以上（選択できる）
  check('テスト用キャラが1体以上（アニメテストで選択可）', API.UNITS.filter(u => u.test).length >= 1, API.UNITS.filter(u => u.test).map(u => u.key));
}
...
console.log('\n=== 100) 通常プレイに test:true βキャラが漏れない ===');
{
  ...
  // ※クマグミ/マシュマロエンジェル/プリンは正式実装（test:true撤去）したのでこのリストから除外
  ['shortcake'].forEach(k => {
    check(k + ' は test:true', API.UNIT_BY_KEY[k].test === true);
    check('x2_' + k + ' は自動生成されない', !API.SPECIALS['x2_' + k]);
    check('STARTER_UNITS に ' + k + ' は含まれない', !API.STARTER_UNITS.includes(k));
  });
  ['kumagumi', 'mangel', 'purin'].forEach(k => {
...
console.log('\n=== 127) カヌレモーラー：潜行→浮上の潜行強襲 ===');
{
  ...
  check('canule は test:true（通常プレイ非登場）', API.UNIT_BY_KEY['canule'].test === true);
