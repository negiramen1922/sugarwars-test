...
console.log('\n=== 90) 経済：カードパック＋通貨（見た目だけ＝対戦は常にフェア） ===');
{
  ...
  check('プールに二つ名（前半）', ids.includes('tp_swift'));
  ...
  // 装備トグル（所持のみ）
  prof.collected={ frame_gold:1, tp_swift:1, ts_king:1, nc_gold:1 }; prof.frame=''; prof.titlePre=''; prof.titleSuf=''; prof.nameColor='';
  ...
  API.equipTitlePre('tp_swift'); check('二つ名前半を装備', API.equippedTitlePre()==='tp_swift');
  API.equipTitleSuf('ts_king'); check('二つ名後半を装備', API.equippedTitleSuf()==='ts_king');
  check('二つ名テキストは前半＋後半（疾風の王）', API.titleText()==='疾風の王', API.titleText());
  check('titleTextOfで任意の組み合わせ', API.titleTextOf('tp_frost','ts_mage')==='氷結の魔導士');
...
{
  const prof = API.myProfileRef;
  prof.coins=300; prof.collected={ frame_gold:2, tp_swift:1, ts_king:1 }; prof.frame='frame_gold'; prof.titlePre='tp_swift'; prof.titleSuf='ts_king'; prof.nameColor='nc_soda'; prof.dailyWinKey='2026-7-1';
  const p=API.buildProfile();
  check('buildProfile: coins', p.coins===300, p.coins);
  check('buildProfile: collected', p.collected.frame_gold===2, p.collected);
  check('buildProfile: frame/二つ名/nameColor', p.frame==='frame_gold'&&p.titlePre==='tp_swift'&&p.titleSuf==='ts_king'&&p.nameColor==='nc_soda');
