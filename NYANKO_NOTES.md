...
- **キャラ別レベル（にゃんこの「にゃんこ強化」相当）**：`save.lv[key]`（1〜`CHAR_LV_MAX`=20）を EXP で1Lvずつ上げると **HPと攻撃が `CHAR_LV_STEP`(6%)ずつ上昇**（`charLvMul`。Lv10=+54%）。`newUnit` が自軍(side'p')のみ hp/atk に倍率を掛ける（敵は対象外）。レベルUPコスト `charLvCost`＝`(25+出撃コスト×0.3)×1.42^(lv-1)`＝**高コストキャラほど・高レベルほど急に重い**。※旧は+8%/×1.35で「最安のクッキーをLv10に上げて連打」が強すぎたため、効き(8→6%)とコスト(1.35→1.42)を調整＝レベルは「勝ちボタン」でなく「後押し」に。UIは **なかま一覧のキャラをタップ→キャラ詳細ダイアログ**（`openCharDetail`/`renderCharDetail`）＝立ち絵・特徴（`UNIT_DESC`）・性能（コスト/HP/攻撃/射程/移動/攻撃速度/再出撃）・レベルUPボタンを表示。見た目でなく**性能に効く恒久強化**（サイフ/タワーと同じEXP経済の追加シンク）。
  - **調整用モード（TEST_MODE）**：`?test`/`?dev` またはホームの「🧪 テストモード」トグルでON＝**全キャラ・全ステージ解放＋バトル開始時お財布/所持金MAX**。加えてキャラ詳細ダイアログに **「🧪 レベル自由設定」パネル**（TEST_MODE時＆解放済みのみ表示）＝スライダー＋クイックボタン(Lv1/5/10/MAX)で**EXP不要でLvを直接セット**（`setCharLevelFree(k,lv)`／1〜MAXへクランプ／通常プレイでは無効）。編成もTEST_MODEなら全キャラから自由に組める＝**編成×レベルを自由にいじってバランス確認**できる。テスト：smoke 26。
...
`node build_nyanko.js` → ヘッドレス smoke（`node nyanko.smoke.js`）で **27ブロック / 184アサーション**（全パス）。メタ関連の検証：サイフ初期上限=100 / `effCap` の Lv0→max 補間 / 城HP=`tHp` / 強化購入でLv↑&EXP消費&コスト逓増 / タワー射程外は当たらない / 解放ゲート（need）＋EXP消費 / TEST_MODEのレベル自由設定（26）/ トゲ3種のslam dmgMul統一（27）。
