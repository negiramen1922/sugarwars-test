...
- **敵ザコの役割（味方の並びに対応させたシンプルな4段）**：複雑な固有効果は付けず、役割を数値と挙動で整理。
  ...
  - **中ボス`m_gboss`（緑・単体）／大ボス`m_boss`（赤）**。大ボスも**単体→単体→ジャンプ範囲**のループ（`slam:{r:64,dmgMul:0.5}`＝範囲は威力半分＝前線を一掃せず倒し切れる）。
- **ボスクラスは前線を維持（`spawnBoss`で `kbCount=2`・旧3）**：ひるみにくくして、**スタッガーの隙に味方がすり抜けて城を叩き「ボスを残したまま勝つ」のを抑える**（＝ボスは倒して突破する壁に）。あわせてボスHP/atkを「倒し切れる壁」へ下方修正（旧`2600+420n`/`52+5.2n`＝すり抜け前提 → `900+150n`/`30+2.6n`）。`slam.dmgMul` で範囲ダメージだけ軽くできる。
