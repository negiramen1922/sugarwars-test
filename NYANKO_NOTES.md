...
- **敵ザコの役割（味方の並びに対応させたシンプルな4段）**：複雑な固有効果は付けず、役割を数値と挙動で整理。
  ...
  - **③`m_thorn`（イガまる・シロトゲ・白）**＝**「単体→単体→ジャンプ範囲」のループ攻撃**（`slam:{r}`＋`slamEvery:3`）。**6面からの主力アタッカー**（旧・緑トゲ`m_gspike`をプールから置き換え。6面以降が簡単だったため白トゲに一本化＋HP250へ強化）。4面はWAVE切替バーストに少しだけ顔見せ（`STAGE_OVERRIDES[4]`）。※`m_gspike`(緑)は定義だけ温存（プール非登場）。
