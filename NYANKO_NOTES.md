...
- **敵ザコの役割（味方の並びに対応させたシンプルな4段）**：複雑な固有効果は付けず、役割を数値と挙動で整理。
  ...
  - **③`m_gspike`（トゲミドリ）**＝雑魚3・**前方AoEアタッカー**（`hits:3`＝前方の敵を近い順に最大3体ヒット・緑属性・紙装甲hp120/atk11/range30）。6面から。
  - **③'`m_thorn`（イガまる・シロトゲ）**＝**ジャンプ衝撃波アタッカー**（`slam:{r}`＝跳ねて着地し半径r内の敵をまとめて叩く radial AoE・白/plain・hp180/atk12/cd1.7）。11面（最初のボス後）から。
  - **④`m_legs`（ノッポし・キモシロカビ足長）**＝高コスト雑魚（高HP950/高atk45・味方のチョコナイト相当）。20面から。
  - **中ボス`m_gboss`（緑）／大ボス`m_boss`（赤・単体強打atk75）**。※ボスの最大3体AoEは climax が理不尽になったため見送り。
- **範囲攻撃2種**：`step`の近接攻撃で ①`D.hits>1`＝射程内・前方の敵を**近い順に最大N体**へ（`area`＝無制限とは別のキャップ版）。②`D.slam:{r}`＝**ジャンプ衝撃波**＝`u.hopT`で跳ね上がり演出＋半径r内の敵をまとめて `attackHit`＋`world.shocks`に広がるリングFXを積む（`SLAM_HOP`秒・`drawUnit`が`hop`で持ち上げ・render が地面沿いの楕円で描画）。テスト：smoke（トゲミドリが3体だけ／イガまるが半径内を巻き込み半径外は無傷＋跳ね/リング生成）。
