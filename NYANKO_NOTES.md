...
- **キャラ別レベル（にゃんこの「にゃんこ強化」相当）**：`save.lv[key]`（1〜`CHAR_LV_MAX`=20）を EXP で1Lvずつ上げると **HPと攻撃が `CHAR_LV_STEP`(6%)ずつ上昇**（`charLvMul`。Lv10=+54%）。`newUnit` が自軍(side'p')のみ hp/atk に倍率を掛ける（敵は対象外）。レベルUPコスト `charLvCost`＝`(25+出撃コスト×0.3)×1.42^(lv-1)`＝**高コストキャラほど・高レベルほど急に重い**。※旧は+8%/×1.35で「最安のクッキーをLv10に上げて連打」が強すぎたため、効き(8→6%)とコスト(1.35→1.42)を調整＝レベルは「勝ちボタン」でなく「後押し」に。UIは **なかま一覧のキャラをタップ→キャラ詳細ダイアログ**（`openCharDetail`/`renderCharDetail`）＝立ち絵・特徴（`UNIT_DESC`）・性能（コスト/HP/攻撃/射程/移動/攻撃速度/再出撃）・レベルUPボタンを表示。見た目でなく**性能に効く恒久強化**（サイフ/タワーと同じEXP経済の追加シンク）。
  - **調整用モード（TEST_MODE）**：`?test`/`?dev` またはホームの「🧪 テストモード」トグルでON＝**全キャラ・全ステージ解放**。**お金/お財布は自動MAXにしない**（＝実際の🍬経済でバランス確認できる。開始🍬/財布Lvは通常どおり`wStart`メタに従う。旧版は開始時にお財布/所持金MAXだった）。加えて2画面で数値を自由設定できる：
    - **キャラ詳細ダイアログ「🧪 レベル自由設定」**（解放済みのみ）＝スライダー＋クイックボタン(Lv1/5/10/MAX)で**EXP不要でLvを直接セット**（`setCharLevelFree(k,lv)`／1〜`CHAR_LV_MAX`へクランプ）。
    - **強化画面「🧪 ◯◯ 自由設定」**（各`UPG`トラックの下）＝スライダー＋クイックボタン(Lv0/中間/MAX)で**EXP不要で強化Lvを直接セット**（`setUpgradeLevelFree(t,lv)`／0〜`u.max`へクランプ）。
    - どちらも **「このLvまで上げるのに本来必要なEXP合計」を表示**（`charLvTotalExp(k,lv)`／`upgTotalExp(t,lv)`＝Lv1/Lv0からの逓増コストの純計算・現在Lvに依らない）＝実プレイでその段階に達する重さの目安。
    - いずれも `TEST_MODE` 以外では無効（通常プレイに影響なし）。編成もTEST_MODEなら全キャラから自由に組める＝**編成×レベル×強化×経済を自由にいじってバランス確認**できる。テスト：smoke 26/28。
...
- **敵の通常攻撃を見せる**：立ち絵の敵は攻撃モーションが無く「いつ殴ったか分からない」ため、`drawFoeSprite` が `u.atkAnim`(攻撃時に立つ)で**狙う方向へグッと突き出す＋横伸び縦縮みのパンチ**を付ける。さらに `attackHit` が有効ヒット以外にも `hitFx`（接触点に小さな火花）を出す＝どちらの攻撃が当たったか見える。
- **交戦のすき間（`CONF.engageGap`=10）**：短射程どうしが立ち絵で食い込むのを軽減。当たり判定(`enemyInRange`/城)・停止位置・範囲攻撃(area/hits)の距離を **`reach=u.range+engageGap`** で共通化（射程の相対差は保ったまま全体に少しすき間）。
- **敵ザコの役割（味方の並びに対応させたシンプルな4段）**：複雑な固有効果は付けず、役割を数値と挙動で整理。
  - **①`m_bump`（ホコリだま・白もこもこ）**＝雑魚1・基本前衛（安い数押し・味方のクッキーナイト相当）。
  - **②`m_neba`（ネバモチ）**＝雑魚2・**タンク**（HP230・足遅い52・**射程20＝短くして最前線に踏み込む**・plain）。シロカビ(bump・射程26)の前でHPを盾に。3面から。旧`m_thorn`/`m_armor`の役割を集約。
  - **③`m_thorn`（イガまる・シロトゲ・白）**＝**「単体→単体→ジャンプ範囲」のループ攻撃**（`slam:{r:66,dmgMul:1.4}`＋`slamEvery:3`）。**6面からの主力アタッカー**（旧・緑トゲ`m_gspike`をプールから置き換え・HP250）。**ジャンプ衝撃波は密集した味方の団子に刺さる＝クッキー連打の対策（散開・混成を促す）**。4面はWAVE切替バーストに少しだけ顔見せ（`STAGE_OVERRIDES[4]`）。※`m_gspike`(緑)は定義だけ温存（プール非登場）。
  - **④`m_legs`（ノッポし・キモシロカビ足長）**＝高コスト雑魚（高HP950/高atk45・味方のチョコナイト相当）。20面から。
  - **中ボス`m_gboss`（緑・単体）／大ボス`m_boss`（赤）**。大ボスも**単体→単体→ジャンプ範囲**のループ（`slam:{r:64,dmgMul:1.4}`）。**トゲ3種（シロ/ミドリ/アカ）の範囲攻撃力(dmgMul)はシロトゲ`m_thorn`基準の1.4で統一**（旧アカトゲは0.5＝範囲半減だったのを、シロトゲに合わせて強化。ミドリトゲ`m_gspike`も1.4）。範囲半径`r`はキャラごと（66/64/52）。
- **ボスクラスは前線を維持（`spawnBoss`で `kbCount=2`・旧3）**：ひるみにくくして、**スタッガーの隙に味方がすり抜けて城を叩き「ボスを残したまま勝つ」のを抑える**（＝ボスは倒して突破する壁に）。あわせてボスHP/atkを「倒し切れる壁」へ下方修正（旧`2600+420n`/`52+5.2n`＝すり抜け前提 → `900+150n`/`30+2.6n`）。`slam.dmgMul` で範囲ダメージを増減できる（現在はトゲ3種とも1.4で統一）。
...
- **引退（プール非登場・定義は数値参照用に温存）**：手描きの`m_swarm`/`m_tank`/`m_big`、立ち絵の`m_armor`（→ネバモチに集約）、射手`m_phage`（後衛は当面出さない方針）。**まだ手描きなのは自爆`m_puff` の1体のみ**。バランスシムでS1〜S9=4/4・S10ボス≈2/4（山）を確認済み。
- **攻撃速度の単位を統一（`atkCdMul`撤廃）**：以前は全体倍率`atkCdMul(1.45)`で攻撃間隔を一律に伸ばし、`cd`は生値だった（表示も生値＝実測とズレ・切りが悪い）。**各`cd`に1.45を畳み込み（1桁に丸め）＝`cd`＝実際の攻撃間隔**に統一。`newUnit`は`cd:D.cd`、キャラ詳細は`D.cd.toFixed(1)秒/回`。全体を速く/遅くしたいときは各`cd`を触る（グローバル倍率はなし）。
