...
- **敵ザコの役割（味方の並びに対応させたシンプルな4段）**：複雑な固有効果は付けず、役割を数値と挙動で整理。
  ...
  - **③`m_gspike`（トゲミドリ）**＝雑魚3・**範囲アタッカー**（`hits:3`＝前方の敵を近い順に最大3体ヒット・緑属性・紙装甲hp120/atk11/range30）。味方の団子に刺さり強いので紙装甲＆低火力に。
...
- **範囲攻撃（`hits:N`）**：`step`の近接攻撃で `D.hits>1` なら射程内・前方の敵を**近い順に最大N体**へ `attackHit`（`D.area`＝無制限とは別・最大数キャップ版）。テスト：smoke（トゲミドリが4体中3体だけヒット）。
- **敵タワーの立ち絵**：敵城は「菌の親」タワー（白属性）の立ち絵 `tower_kin` を `drawCastle(side==='e')` で描画（味方城は従来の手描き青）。地面に接地・城位置(`castleInset`)を中心に132px。
- **引退（プール非登場・定義は数値参照用に温存）**：手描きの`m_swarm`/`m_tank`/`m_big`、立ち絵の`m_thorn`/`m_armor`（→ネバモチに集約）、射手`m_phage`（後衛は当面出さない方針）。**まだ手描きなのは自爆`m_puff` の1体のみ**。バランスシムでS1〜S9=4/4・S10ボス=3/4（山）を確認済み。
