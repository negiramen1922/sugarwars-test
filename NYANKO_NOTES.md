...
- **敵ザコの役割（味方の並びに対応させたシンプルな4段）**：複雑な固有効果は付けず、役割を数値と挙動で整理。
  ...
  - **中ボス`m_gboss`（ミドリのぬし・緑）＝AoEなしの単体特化**：**重い一撃(atk高)＋遅い攻撃(cd長)**で正面の1体を狙い撃つ壁役（`slam`なし）。DEF＝atk66/cd3.4、5面の`mini`cfg＝atk`50+n*5`/cd3.4（`spawnBoss`が`cfg.cd`/`cfg.atk`で上書き）。**方針：安易に全敵へAoEを付けない**＝団子割りはトゲ系(`slam`)に任せ、中ボスは「単体重火力の脅威」に役割分担。将来はステージ6以降の最終wave等に**同じ敵として再登場**させても identity が揃うようDEF自体を単体重火力にしてある。／**大ボス`m_boss`（赤）**は**単体→単体→ジャンプ範囲**のループ（`slam:{r:64,dmgMul:1.4}`）。**トゲ3種（シロ/ミドリ/アカ）の範囲攻撃力(dmgMul)はシロトゲ`m_thorn`基準の1.4で統一**（旧アカトゲは0.5＝範囲半減だったのを、シロトゲに合わせて強化。ミドリトゲ`m_gspike`も1.4）。範囲半径`r`はキャラごと（66/64/52）。
...
自動生成の上から**ステージ個別に手動オーバーライド**できる（`STAGES` 生成直後に `STAGE_OVERRIDES[id](s)` を適用）。1面がその一例（単一ゲージへ上書き）。現在は **4面**（バーストにシロトゲ顔見せ）／**5面**（ブレイクの出現パターンにシロトゲ`m_thorn`を織り込む＝クッキー団子のごり押し対策・`w.heavy`は無効化して完全固定パターン化）を上書き。触れるノブ：

> **【バランスメモ】クッキー単騎ごり押しについて（sim検証済み）**：クッキーLv5単騎でS5を時間で押し切れる問題。sim結果＝**シロトゲをブレイクに1/4入れる程度では止まらない**（単騎も約53秒で勝つ）。確実に止めるにはブレイク2/4＋通常waveにも入れる必要があり、その場合**S5全体が2分級のスローペース**になる副作用が出る（混成も遅くなる）。**お金の生産を遅くしても無限時間のごり押しは止まらない**（時間が延びるだけ）＝経済ノブはこの問題に効かない。中ボスは単体攻撃なので団子割りには寄与しない（脅威=単体重火力の identity 変更のみ）。**現状は「ブレイクに1/4のシロトゲ＝とりあえずの種まき」**。どこまで密度を上げる（＝どこまでS5を重くする）かは要判断。
...
`node build_nyanko.js` → ヘッドレス smoke（`node nyanko.smoke.js`）で **30ブロック / 211アサーション**（全パス）。メタ関連の検証：サイフ初期上限=100 / `effCap` の Lv0→max 補間 / 城HP=`tHp` / 強化購入でLv↑&EXP消費&コスト逓増 / タワー射程外は当たらない / 解放ゲート（need）＋EXP消費 / TEST_MODEはお金非MAX＆レベル自由設定（7・26）/ トゲ3種のslam dmgMul統一（27）/ 強化Lv自由設定＋到達EXP合計の計算（28）/ 敵湧きの決定的ループ（29）。
